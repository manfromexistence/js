...
## DX JavaScript Branch: Package Metadata Machine Cache

This `dev` branch carries the DX package metadata `.machine` cache integration. For packages inside `node_modules`, the resolver can use DX-generated package metadata instead of paying the normal JSON read path for every package metadata load.
...
| ------ | -----------------: | -----------: | --------------: | ------------------------: | -------: |
| small  |        `132,022` B |   `14.143ms` |       `7.383ms` | `.machine` faster `47.8%` | `1.916x` |
| medium |      `2,099,346` B |   `30.103ms` |      `13.940ms` | `.machine` faster `53.7%` | `2.159x` |
| large  |     `16,781,714` B |   `70.668ms` |      `27.903ms` | `.machine` faster `60.5%` | `2.533x` |

The proof records explicit machine-cache hits for package metadata reads:

| case   | machine proof                                   |
| ------ | ----------------------------------------------- |
| small  | `64/64` package metadata reads hit `.machine`   |
| medium | `128/128` package metadata reads hit `.machine` |
| large  | `256/256` package metadata reads hit `.machine` |

The result verifies that DX `.machine` is wired into the resolver path where package metadata JSON was being read, and that this local fork is faster than its own local JSON path on the measured package metadata fixture sizes.

The implementation remains deliberately narrow:

- package metadata inside `node_modules` can use the DX machine-cache path;
- invalid, missing, or unsupported machine-cache entries fall back to the existing JSON behavior;
- proof logging is opt-in through the benchmark environment and is not part of normal runtime output.
