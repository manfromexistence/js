...
describe("Bun.build", () => {
  ...
  test("bytecode sidecar cache hydrates and invalidates against source", async () => {
    const dir = tempDirWithFiles("bun-build-api-bytecode-invalidation", {
      "package.json": `{}`,
      "index.ts": `console.log("first");`,
    });
    ...
    const first = await runBuiltFile();
    expect(first.exitCode).toBe(0);
    expect(first.stdout).toBe("first\n");
    expect(first.stderr).toContain("[Disk Cache] Cache hit for sourceCode");

    writeFileSync(entry!.path, `console.log("second");\n`);
    const second = await runBuiltFile();
    expect(second.exitCode).toBe(0);
    expect(second.stdout).toBe("second\n");
    expect(second.stderr).toContain("[Disk Cache] Cache miss for sourceCode");
  });
