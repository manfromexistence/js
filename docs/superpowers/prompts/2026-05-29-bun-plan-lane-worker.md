# Bun PLAN.md Goal Mode Lane Worker Prompt

Paste this prompt into each of the 6 Bun worker chats. Every worker runs the same prompt. The allocator assigns a lane by counter, prints the exact PLAN.md tasks for that lane, and the worker then keeps going in Codex Desktop Goal Mode until the lane is complete or honestly blocked.

```text
Use [@superpowers](plugin://superpowers@openai-curated) first.

You are a Codex Desktop GPT-5.5 extra-high implementation worker for G:\Dx\js.
...
Goal Mode setup:
- Create a goal for your assigned Bun PLAN.md lane.
...
## Coordinator Quick Command

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File "G:\Dx\js\scripts\codex\claim-bun-plan-lane.ps1" -ShowAll
```
