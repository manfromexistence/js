...
const DEFAULT_RELEASE_CODEGEN_UNITS = "16";
const RELEASE_CODEGEN_UNITS_ENV = "BUN_DX_RELEASE_CODEGEN_UNITS";

function releaseCodegenUnitsForNoLtoBuild(): string {
  const override = process.env[RELEASE_CODEGEN_UNITS_ENV]?.trim();
  if (override === undefined || override === "") return DEFAULT_RELEASE_CODEGEN_UNITS;
  if (/^[1-9]\d*$/.test(override)) return override;
  throw new Error(`${RELEASE_CODEGEN_UNITS_ENV} must be a positive integer, got ${JSON.stringify(override)}`);
}
...
export function emitRust(n: Ninja, cfg: Config, inputs: RustBuildInputs): string[] {
  ...
  const releaseWithoutCrossLanguageLto = cfg.release && !cfg.lto && !cfg.asan;
  if (cfg.crossLangLto) {
  ...
  } else if (releaseWithoutCrossLanguageLto) {
    // Native/local release profile must not accidentally inherit Cargo.toml
    // fat LTO. `profiles.ts` declares plain `release` as `lto: false`, but
    // Cargo.toml keeps `[profile.release] lto = "fat"` for the LTO-capable
    // lanes unless the ninja edge overrides it here. Without this, the
    // default Windows `--profile=release --ci=on -j6` build can spend the Rust
    // staticlib step in serial fat-LTO codegen or fail without diagnostics.
    //
    // Keep the normal local default at 16 CGUs, but allow proof builds on
    // memory-constrained Windows machines to lower this without editing
    // Cargo.toml or reducing the user-requested ninja `-j` value.
    env.CARGO_PROFILE_RELEASE_LTO = "off";
    env.CARGO_PROFILE_RELEASE_CODEGEN_UNITS = releaseCodegenUnitsForNoLtoBuild();
  }
