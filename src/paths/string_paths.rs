...
pub fn normalize_slashes_only<'a>(
    ...
    let Some(first_slash) = memchr::memchr(undesired_slash, utf8) else {
        return utf8;
    };

    buf[..utf8.len()].copy_from_slice(utf8);
    let mut search_start = first_slash;
    while let Some(slash) = memchr::memchr(undesired_slash, &buf[search_start..utf8.len()]) {
        let slash = search_start + slash;
        buf[slash] = desired_slash;
...
pub fn path_contains_node_modules_folder(path: &[u8]) -> bool {
    memchr::memmem::find(path, crate::NODE_MODULES_NEEDLE).is_some()
}
...
#[cfg(test)]
mod tests {
    use super::path_contains_node_modules_folder;

    fn path(parts: &[&[u8]]) -> Vec<u8> {
        let mut out = Vec::new();
    ...
    #[test]
    fn node_modules_folder_scan_matches_segment_boundaries() {
        assert!(path_contains_node_modules_folder(&path(&[
            b"repo",
            b"node_modules",
            b"pkg",
            b"index.js",
        ...
        assert!(!path_contains_node_modules_folder(b"node_modules"));
        assert!(!path_contains_node_modules_folder(&path(&[
            b"repo",
            b"node_modules",
        ])));
