...
impl SavedSourceMap {
    #[inline]
    fn clear_last_ism_for_key(&mut self, key: u64) {
        if self.last_path_hash == key {
            self.last_ism = None;
        }
    }

    #[inline]
    fn invalidate_internal_source_map(&mut self, ism: InternalSourceMap) {
        self.find_cache.invalidate(ism.data);
        if let Some(cached) = self.last_ism {
            if cached.data == ism.data {
                self.last_ism = None;
            }
        }
    }
    ...
    #[inline]
    pub fn find_mapping_with_cache(
        &mut self,
        path: &[u8],
        map: &ParsedSourceMap,
        line: Ordinal,
        column: Ordinal,
    ) -> Option<SourceMap::Mapping> {
        if let Some(ism) = map.internal {
            self.last_path_hash = hash(path);
            self.last_ism = Some(ism);
            return ism.find_with_cache(line, column, &mut self.find_cache);
        }

        map.find_mapping(line, column)
    }

    fn release_table_value(&mut self, key: u64, value: Value) {
        self.clear_last_ism_for_key(key);
        if let Some(parsed_source_map) = value.get::<ParsedSourceMap>() {
            if let Some(ism) = unsafe { (*parsed_source_map).internal } {
                self.invalidate_internal_source_map(ism);
            }
            // SAFETY: pointer was stored by us and is live until removed.
    ...
    fn remove_value_if_locked(
        &mut self,
        key: u64,
        should_remove: impl FnOnce(Value) -> bool,
    ) -> bool {
        let removed_value = {
            ...
            if !should_remove(Value::from(Some(ptr))) {
                return false;
            }
        ...
        self.release_table_value(key, removed_value);
        true
    }
    ...
    pub fn remove_dev_server_source_provider(
        ...
        self.lock();
        let key = hash(path);
        self.remove_value_if_locked(key, |old_value| {
            if let Some(prov) = old_value.get::<DevServerSourceProvider>() {
                return core::ptr::eq(prov.cast::<c_void>(), opaque_source_provider);
            }

            let Some(parsed) = old_value.get::<ParsedSourceMap>() else {
                return false;
            };
            // SAFETY: `parsed` was stored by us and is live while in the table.
            if let Some(prov) = unsafe { (*parsed).underlying_provider }.provider() {
                return core::ptr::eq(prov.ptr(), opaque_source_provider);
            }
            false
        });
        self.unlock();
...
impl SavedSourceMap {
    ...
    pub fn put_value(&mut self, path: &[u8], value: Value) -> Result<(), bun_core::Error> {
        ...
        let old_value = self
            .map_mut()
            .insert(key, value.ptr())
            .map(|ptr| Value::from(Some(ptr)));

        if let Some(old_value) = old_value {
            self.release_table_value(key, old_value);
        } else {
            self.clear_last_ism_for_key(key);
        }
        self.unlock();
    ...
    fn get_with_content(
        ...
        if tag == Value::case::<InternalSourceMap>() {
            ...
            *mapping = Value::init(Arc::into_raw(Arc::clone(&result))).ptr();
            ...
            let removed = self.remove_value_if_locked(h, |value| {
                if let Some(current) = value.get::<SourceProviderMap>() {
                    return core::ptr::eq(current, ptr);
                }
                false
            });
            ...
            // does not have a valid source map. let's not try again
            let removed = self.remove_value_if_locked(h, |value| {
                if let Some(current) = value.get::<BakeSourceProvider>() {
                    return core::ptr::eq(current, ptr);
                }
                false
            });
            ...
            // does not have a valid source map. let's not try again
            let removed = self.remove_value_if_locked(h, |value| {
                if let Some(current) = value.get::<DevServerSourceProvider>() {
                    return core::ptr::eq(current, ptr);
                }
                false
            });
    ...
    pub fn resolve_mapping(
        ...
        let mapping = match parse.mapping {
            ...
            None => self.find_mapping_with_cache(path, map.as_ref(), line, column)?,
        };
