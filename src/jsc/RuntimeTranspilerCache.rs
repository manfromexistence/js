...
use bun_ast::Source;
use bun_core::{env_var, FeatureFlags};
...
impl Metadata {
    ...
    pub fn validate_payload_layout(&self, stat_size: u64) -> Result<(), bun_core::Error> {
        let metadata_size = Metadata::SIZE as u64;
        if self.output_byte_offset != metadata_size {
            return Err(bun_core::err!(MissingData));
        }

        let output_end =
            checked_payload_end(self.output_byte_offset, self.output_byte_length, stat_size)?;
        if self.sourcemap_byte_offset != output_end {
            return Err(bun_core::err!(MissingData));
        }

        let sourcemap_end = checked_payload_end(
            self.sourcemap_byte_offset,
            self.sourcemap_byte_length,
            stat_size,
        )?;
        if self.esm_record_byte_offset != sourcemap_end {
            return Err(bun_core::err!(MissingData));
        }

        checked_payload_end(
            self.esm_record_byte_offset,
            self.esm_record_byte_length,
            stat_size,
        )?;

        if matches!(self.output_encoding, Encoding::UTF16) && self.output_byte_length % 2 != 0 {
            return Err(bun_core::err!(MissingData));
        }

        self.output_byte_length_usize()?;
        self.sourcemap_byte_length_usize()?;
        self.esm_record_byte_length_usize()?;

        Ok(())
    }

    #[inline]
    fn output_byte_length_usize(&self) -> Result<usize, bun_core::Error> {
        payload_len_usize(self.output_byte_length)
    }

    #[inline]
    fn sourcemap_byte_length_usize(&self) -> Result<usize, bun_core::Error> {
        payload_len_usize(self.sourcemap_byte_length)
    }

    #[inline]
    fn esm_record_byte_length_usize(&self) -> Result<usize, bun_core::Error> {
        payload_len_usize(self.esm_record_byte_length)
    }
}
...
fn checked_payload_end(offset: u64, len: u64, stat_size: u64) -> Result<u64, bun_core::Error> {
    let end = offset
        .checked_add(len)
...
fn payload_len_usize(len: u64) -> Result<usize, bun_core::Error> {
    usize::try_from(len).map_err(|_| bun_core::err!(MissingData))
}
...
impl Entry {
    ...
    pub fn save(
        ...
        let _close_guard = sys::CloseOnDrop::new(tmpfile.fd);
        {
            ...
            let payloads: [&[u8]; 4] = [metadata_bytes, output_bytes, sourcemap, esm_record];
            let end_position =
                Metadata::SIZE + output_bytes.len() + sourcemap.len() + esm_record.len();

            #[cfg(debug_assertions)]
            {
                let total: usize = payloads.iter().map(|bytes| bytes.len()).sum();
            ...
            pwritev_exact(tmpfile.fd, &payloads, end_position)?;
    ...
    pub fn load(&mut self, file: &sys::File) -> Result<(), bun_core::Error> {
        ...
        self.metadata.validate_payload_layout(stat_size)?;
        ...
        let output_len = self.metadata.output_byte_length_usize()?;
        let sourcemap_len = self.metadata.sourcemap_byte_length_usize()?;
        let esm_record_len = self.metadata.esm_record_byte_length_usize()?;
        let mut sourcemap = (self.metadata.sourcemap_byte_length > 0)
            .then(|| Box::<[u8]>::new_uninit_slice(sourcemap_len));
        let mut esm_record = (self.metadata.esm_record_byte_length > 0)
            .then(|| Box::<[u8]>::new_uninit_slice(esm_record_len));
        ...
        self.output_code = if self.metadata.output_byte_length == 0 {
            read_entry_payloads(
                ...
                &self.metadata,
                None,
                sourcemap.as_deref_mut(),
                esm_record.as_deref_mut(),
            )?;
        ...
        if let Some(sourcemap) = sourcemap {
            let sourcemap = unsafe { sourcemap.assume_init() };
            if self.metadata.sourcemap_hash != 0 {
        ...
        if let Some(esm_record) = esm_record {
            let esm_record = unsafe { esm_record.assume_init() };
...
fn read_entry_payloads(
    file: &sys::File,
    metadata: &Metadata,
    output: Option<&mut [u8]>,
    sourcemap: Option<&mut [core::mem::MaybeUninit<u8>]>,
    esm_record: Option<&mut [core::mem::MaybeUninit<u8>]>,
) -> Result<(), bun_core::Error> {
    if output.as_ref().map_or(0, |bytes| bytes.len()) as u64 != metadata.output_byte_length
        || sourcemap.as_ref().map_or(0, |bytes| bytes.len()) as u64
            != metadata.sourcemap_byte_length
        || esm_record.as_ref().map_or(0, |bytes| bytes.len()) as u64
            != metadata.esm_record_byte_length
    {
        return Err(bun_core::err!(MissingData));
    }
    ...
    #[cfg(unix)]
    {
        ...
        let mut sourcemap = sourcemap;
        let mut esm_record = esm_record;
        let mut vecs_buf = [const { core::mem::MaybeUninit::<sys::PlatformIoVec>::uninit() }; 3];
        let mut segment_offsets = [0u64; 3];
        let mut segment_lengths = [0usize; 3];
        ...
        macro_rules! add_segment {
            ($bytes:expr, $offset:expr) => {{
                ...
                if !bytes.is_empty() {
                    let offset = $offset;
                    segment_offsets[vecs_i] = offset;
                    segment_lengths[vecs_i] = bytes.len();
                    total_len += bytes.len();
                    vecs_buf[vecs_i].write(sys::platform_iovec_create(bytes));
                    vecs_i += 1;
                }
            }};
        }
        ...
        if let Some(bytes) = output.as_deref_mut() {
            add_segment!(bytes, metadata.output_byte_offset);
        }
        if let Some(bytes) = sourcemap.as_deref_mut() {
            add_segment!(
                uninit_slice_as_bytes_mut(bytes),
                metadata.sourcemap_byte_offset
            );
        }
        if let Some(bytes) = esm_record.as_deref_mut() {
            add_segment!(
                uninit_slice_as_bytes_mut(bytes),
                metadata.esm_record_byte_offset
            );
        }

        let can_vector = (1..vecs_i).all(|i| {
            match segment_offsets[i - 1].checked_add(segment_lengths[i - 1] as u64) {
                Some(next_offset) => next_offset == segment_offsets[i],
                None => false,
            }
        });

        if vecs_i > 1 && can_vector {
            let vecs = unsafe {
                core::slice::from_raw_parts_mut(
                    vecs_buf.as_mut_ptr().cast::<sys::PlatformIoVec>(),
                    vecs_i,
                )
            };
            return preadv_exact(file, vecs, segment_offsets[0], total_len);
        }

        return read_entry_payloads_fallback(file, metadata, output, sourcemap, esm_record);
    }
    ...
    #[cfg(not(unix))]
    {
        ...
        let mut sourcemap = sourcemap;
        let mut esm_record = esm_record;
        if read_entry_payloads_contiguous(
            file,
            metadata,
            &mut output,
            &mut sourcemap,
        ...
        read_entry_payloads_fallback(file, metadata, output, sourcemap, esm_record)
    }
}

#[cfg(not(unix))]
const CONTIGUOUS_READ_MIN_BYTES: usize = 256 * 1024;

#[cfg(not(unix))]
fn read_entry_payloads_contiguous(
    file: &sys::File,
    metadata: &Metadata,
    output: &mut Option<&mut [u8]>,
    sourcemap: &mut Option<&mut [core::mem::MaybeUninit<u8>]>,
    esm_record: &mut Option<&mut [core::mem::MaybeUninit<u8>]>,
) -> Result<bool, bun_core::Error> {
    let segments = [
        output
            .as_ref()
            .map(|bytes| (metadata.output_byte_offset, bytes.len())),
        sourcemap
            .as_ref()
            .map(|bytes| (metadata.sourcemap_byte_offset, bytes.len())),
        esm_record
            .as_ref()
            .map(|bytes| (metadata.esm_record_byte_offset, bytes.len())),
    ];
    ...
    for segment in segments.into_iter().flatten().filter(|(_, len)| *len > 0) {
        let (offset, len) = segment;
        if segment_count == 0 {
            first_offset = offset;
            next_offset = offset;
        }
        ...
        let Some(next) = offset.checked_add(len as u64) else {
            return Ok(false);
        };
        let Some(total) = total_len.checked_add(len) else {
            return Ok(false);
        };
        next_offset = next;
        total_len = total;
    ...
    if total_len < CONTIGUOUS_READ_MIN_BYTES {
        return Ok(false);
    }

    let mut scratch = vec![0; total_len];
    pread_exact(file, &mut scratch, first_offset)?;

    let mut cursor = 0usize;
    if let Some(bytes) = output.as_deref_mut() {
        let end = cursor + bytes.len();
        bytes.copy_from_slice(&scratch[cursor..end]);
        cursor = end;
    }
    if let Some(bytes) = sourcemap.as_deref_mut() {
        let bytes = uninit_slice_as_bytes_mut(bytes);
        let end = cursor + bytes.len();
        bytes.copy_from_slice(&scratch[cursor..end]);
        cursor = end;
    }
...
fn read_entry_payloads_fallback(
    file: &sys::File,
    metadata: &Metadata,
    output: Option<&mut [u8]>,
    sourcemap: Option<&mut [core::mem::MaybeUninit<u8>]>,
    ...
    if let Some(bytes) = output {
        pread_exact(file, bytes, metadata.output_byte_offset)?;
    }
    if let Some(bytes) = sourcemap {
        pread_exact(
            file,
            uninit_slice_as_bytes_mut(bytes),
            metadata.sourcemap_byte_offset,
...
fn pwritev_exact(fd: Fd, payloads: &[&[u8]], total_len: usize) -> Result<(), bun_core::Error> {
    pwritev_exact_with(fd, payloads, total_len, sys::pwritev)
}

#[inline]
fn pwritev_exact_with<W>(
    fd: Fd,
    payloads: &[&[u8]],
    total_len: usize,
    mut write_vectored_at: W,
) -> Result<(), bun_core::Error>
where
    W: FnMut(Fd, &[sys::PlatformIoVecConst], i64) -> sys::Maybe<usize>,
{
    ...
    while written_total < total_len {
        ...
        let vecs_i = remaining_pwritev_iovecs(payloads, written_total, &mut vecs_buf);
        debug_assert!(vecs_i > 0);
        if vecs_i == 0 {
            return Err(bun_core::err!(WriteFailed));
        }

        let written = write_vectored_at(fd, &vecs_buf[0..vecs_i], position)?;
        if written == 0 {
            return Err(bun_core::err!(WriteFailed));
        }

        let remaining = total_len - written_total;
        let written = written.min(remaining);
...
fn remaining_pwritev_iovecs(
    payloads: &[&[u8]],
    mut skip: usize,
    vecs_buf: &mut [sys::PlatformIoVecConst],
) -> usize {
    let mut vecs_i = 0usize;
    ...
    for payload in payloads.iter().copied() {
        ...
        debug_assert!(vecs_i < vecs_buf.len());
        vecs_buf[vecs_i] = sys::platform_iovec_const_create(payload);
        vecs_i += 1;
    }
...
fn pread_exact(file: &sys::File, dst: &mut [u8], offset: u64) -> Result<(), bun_core::Error> {
...
fn uninit_slice_as_bytes_mut(buf: &mut [core::mem::MaybeUninit<u8>]) -> &mut [u8] {
    // SAFETY: `MaybeUninit<u8>` and `u8` have identical size/align, and
    // callers only pass the resulting slice to read syscalls that write bytes.
...
fn preadv_exact(
    ...
    while remaining > 0 {
        let read = sys::preadv(file.handle, &vecs[first..], position)?;
        if read == 0 {
            return Err(bun_core::err!(MissingData));
        }

        debug_assert!(read <= remaining);
        let read = read.min(remaining);
        position += read as i64;
        remaining -= read;
        advance_iovecs(vecs, &mut first, read);
    }
...
#[cfg(unix)]
fn advance_iovecs(vecs: &mut [sys::PlatformIoVec], first: &mut usize, mut read: usize) {
    while read > 0 {
        let len = vecs[*first].iov_len;
        if read < len {
            let base = vecs[*first].iov_base.cast::<u8>();
            vecs[*first].iov_base = unsafe { base.add(read).cast() };
            vecs[*first].iov_len -= read;
            break;
        }
...
mod tests {
    ...
    #[cfg(unix)]
    fn iovec_len(vec: &sys::PlatformIoVecConst) -> usize {
        vec.len
    }
    ...
    fn iovec_base(vec: &sys::PlatformIoVecConst) -> *const u8 {
        vec.base
    }

    #[test]
    fn remaining_pwritev_iovecs_starts_at_unwritten_payload_byte() {
        let metadata = b"metadata";
        let output = b"output";
        let sourcemap = b"sourcemap";
        let esm_record = b"esm";
        let payloads: [&[u8]; 4] = [metadata, output, sourcemap, esm_record];
        let mut vecs: [sys::PlatformIoVecConst; 4] = bun_core::ffi::zeroed();

        let count = remaining_pwritev_iovecs(&payloads, 0, &mut vecs);
        assert_eq!(count, 4);
        assert_eq!(iovec_base(&vecs[0]), metadata.as_ptr());
        assert_eq!(iovec_len(&vecs[0]), metadata.len());

        let count = remaining_pwritev_iovecs(&payloads, metadata.len(), &mut vecs);
        assert_eq!(count, 3);
        assert_eq!(iovec_base(&vecs[0]), output.as_ptr());
        assert_eq!(iovec_len(&vecs[0]), output.len());

        let output_skip = 2;
        let count = remaining_pwritev_iovecs(&payloads, metadata.len() + output_skip, &mut vecs);
        assert_eq!(count, 3);
        assert_eq!(iovec_base(&vecs[0]), unsafe {
            output.as_ptr().add(output_skip)
        });
        assert_eq!(iovec_len(&vecs[0]), output.len() - output_skip);

        let sourcemap_skip = 3;
        let count = remaining_pwritev_iovecs(
            &payloads,
            metadata.len() + output.len() + sourcemap_skip,
            &mut vecs,
        );
        assert_eq!(count, 2);
        assert_eq!(iovec_base(&vecs[0]), unsafe {
            sourcemap.as_ptr().add(sourcemap_skip)
        });
        assert_eq!(iovec_len(&vecs[0]), sourcemap.len() - sourcemap_skip);

        let total_len: usize = payloads.iter().map(|payload| payload.len()).sum();
        assert_eq!(remaining_pwritev_iovecs(&payloads, total_len, &mut vecs), 0);
    }

    #[test]
    fn remaining_pwritev_iovecs_ignores_empty_payloads() {
        let output = b"output";
        let payloads: [&[u8]; 4] = [b"", output, b"", b""];
        let mut vecs: [sys::PlatformIoVecConst; 4] = bun_core::ffi::zeroed();
    ...
    #[test]
    fn pwritev_exact_retries_simulated_short_writes_without_rewriting_bytes() {
        let metadata = b"metadata";
        let output = b"output";
        let sourcemap = b"sourcemap";
        let esm_record = b"esm";
        ...
        pwritev_exact_with(
            ...
            &payloads,
            total_len,
            |_, vecs, position| -> sys::Maybe<usize> {
                let position = usize::try_from(position).expect("non-negative position");
                let requested: usize = vecs.iter().map(iovec_len).sum();
                let cap = requested.min(3);
                assert!(cap > 0);

                if written.len() < position + cap {
                    written.resize(position + cap, 0);
                }
                ...
                for vec in vecs {
                    ...
                    let len = iovec_len(vec).min(cap - copied);
                    let bytes = unsafe { core::slice::from_raw_parts(iovec_base(vec), len) };
                    let start = position + copied;
                    written[start..start + len].copy_from_slice(bytes);
                    copied += len;
                }

                calls.push((position, copied));
                Ok(copied)
            },
        )
        .expect("short writes are retried until the complete entry is written");

        assert_eq!(written, expected);
        assert!(calls.len() > 1);
        let mut cursor = 0usize;
        for (position, copied) in calls {
