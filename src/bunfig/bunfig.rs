...
/// Boundary for callers that already have a parser-produced bunfig AST.
pub(crate) struct TrustedBunfig<'a> {
    expr: Expr,
    bump: &'a Bump,
}

impl<'a> TrustedBunfig<'a> {
    #[inline]
    pub(crate) fn from_parser(expr: Expr, bump: &'a Bump) -> Self {
        Self { expr, bump }
    }
}
...
impl Bunfig {
    pub fn parse(
        ...
        if bun_resolver::dx_machine_cache::shadow_probe_enabled() {
            let _dx_machine_shadow_probe =
                bun_resolver::dx_machine_cache::shadow_probe_source_or_warn(
                    bun_resolver::dx_machine_cache::DxMachineCacheKind::Bunfig,
                    source.path.text,
                    source.contents(),
                );
        }
        ...
        Self::parse_trusted(
            ...
            ctx,
            log,
            TrustedBunfig::from_parser(expr, &bump),
        )
    }

    pub(crate) fn parse_trusted(
        cmd: CommandTag,
        source: &bun_ast::Source,
        ctx: &mut ContextData,
        log: &mut bun_ast::Log,
        trusted: TrustedBunfig<'_>,
    ) -> Result<(), bun_core::Error> {
        ...
        // writes through `self.ctx.args` directly while callers provide the
        // disjoint `Log` borrow explicitly.
