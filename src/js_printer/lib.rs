...
pub mod __gated_printer {
    ...
    #[inline]
    fn print_non_negative_float_to(mut print: impl FnMut(&[u8]), float: f64) {
        ...
        if float < (u64::MAX >> 12) as f64 /* maxInt(u52) */ && is_integer {
            ...
            print(bun_core::fmt::itoa(&mut buf, val));
            return;
        }

        let mut buf = ryu::Buffer::new();
        print(buf.format_finite(float).as_bytes());
    }
        ...
        W: WriterTrait,
    {
        ...
        pub fn print_non_negative_float(&mut self, float: f64) {
            print_non_negative_float_to(|bytes| self.print(bytes), float);
        }
    ...
    #[cfg(test)]
    mod tests {
        use super::print_non_negative_float_to;

        fn format_non_negative_float(float: f64) -> String {
            let mut out = Vec::new();
            print_non_negative_float_to(|bytes| out.extend_from_slice(bytes), float);
            String::from_utf8(out).expect("printer number output is ASCII")
        }

        #[test]
        fn non_negative_integer_output_stays_on_itoa_path() {
            assert_eq!(format_non_negative_float(0.0), "0");
            assert_eq!(format_non_negative_float(42.0), "42");
            assert_eq!(format_non_negative_float(10_000.0), "1e4");
            assert_eq!(format_non_negative_float(1_000_000_000.0), "1e9");
            assert_eq!(format_non_negative_float(10_000_000_000.0), "10000000000");
        }

        #[test]
        fn non_integer_float_output_uses_short_round_trip_decimal() {
            for value in [0.1, 1.2345, 1e20 + 0.5, f64::MIN_POSITIVE] {
                let printed = format_non_negative_float(value);
                let reparsed: f64 = printed.parse().expect("ryu output parses as f64");

                assert_eq!(reparsed, value);
                assert_eq!(printed, ryu::Buffer::new().format_finite(value));
            }
        }
    }
} // mod __gated_printer
