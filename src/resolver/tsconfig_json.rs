...
/// Boundary for callers that already have a parser-produced tsconfig AST.
pub(crate) struct TrustedTsconfigJson {
    expr: bun_ast::Expr,
}

impl TrustedTsconfigJson {
    #[inline]
    pub(crate) fn from_parser(expr: bun_ast::Expr) -> Self {
        Self { expr }
    }
}

pub(crate) type PathTargets = SmallVec<[Box<[u8]>; 2]>;
pub(crate) type PathsMap = StringArrayHashMap<PathTargets>;
pub(crate) type PathPatternTargets = SmallVec<[TSConfigPathTarget; 2]>;
pub(crate) type PathPatterns = SmallVec<[TSConfigPathPattern; 4]>;

#[derive(Clone)]
pub(crate) struct TSConfigPathTarget {
    pub prefix: Box<[u8]>,
    pub suffix: Box<[u8]>,
    pub has_wildcard: bool,
}

impl TSConfigPathTarget {
    fn new(target: &[u8]) -> Self {
        if let Some(star) = strings::index_of_char(target, b'*') {
            let star = star as usize;
            return Self {
                prefix: Box::from(&target[..star]),
                suffix: Box::from(strings::trim_left(&target[star..], b"*")),
                has_wildcard: true,
            };
        }

        Self {
            prefix: Box::from(target),
            suffix: Box::default(),
            has_wildcard: false,
        }
    }
...
#[derive(Clone)]
pub(crate) struct TSConfigPathPattern {
    pub prefix: Box<[u8]>,
    pub suffix: Box<[u8]>,
    pub targets: PathPatternTargets,
}

impl TSConfigPathPattern {
    fn new(key: &[u8], targets: &PathTargets) -> Option<Self> {
        let star = strings::index_of_char(key, b'*')? as usize;
        Some(Self {
            prefix: Box::from(&key[..star]),
            suffix: Box::from(&key[star + 1..]),
            targets: targets
                .iter()
                .map(|target| TSConfigPathTarget::new(target.as_ref()))
                .collect(),
        })
    }
    ...
    pub(crate) fn matches(&self, path: &[u8]) -> bool {
        if path.len() < self.prefix.len() + self.suffix.len() {
            return false;
        }
...
pub struct TSConfigJSON {
    ...
    pub paths: PathsMap,
    pub path_patterns: PathPatterns,
    pub exact_path_min_len: u32,
    pub exact_path_max_len: u32,
...
impl Default for TSConfigJSON {
    fn default() -> Self {
        Self {
            ...
            path_patterns: PathPatterns::new(),
            exact_path_min_len: 0,
            exact_path_max_len: 0,
...
impl TSConfigJSON {
    fn rebuild_path_patterns(&mut self) {
        self.path_patterns.clear();
        let mut exact_path_min_len = u32::MAX;
        let mut exact_path_max_len = 0u32;
        for (key, values) in self.paths.iter() {
            if let Some(pattern) = TSConfigPathPattern::new(key.as_ref(), values) {
                self.path_patterns.push(pattern);
            } else {
                let key_len = u32::try_from(key.as_ref().len()).unwrap_or(u32::MAX);
                exact_path_min_len = exact_path_min_len.min(key_len);
                exact_path_max_len = exact_path_max_len.max(key_len);
            }
        }
        if exact_path_max_len == 0 {
            exact_path_min_len = 0;
        }
        self.exact_path_min_len = exact_path_min_len;
        self.exact_path_max_len = exact_path_max_len;

        if self.path_patterns.len() > 1 {
            self.path_patterns.sort_by(|a, b| {
                b.prefix
                    .len()
                    .cmp(&a.prefix.len())
    ...
    pub fn parse(
        ...
        if crate::dx_machine_cache::shadow_probe_enabled() {
            let _dx_machine_shadow_probe = crate::dx_machine_cache::shadow_probe_source_or_warn(
                crate::dx_machine_cache::DxMachineCacheKind::Tsconfig,
                source.path.text,
                source.contents(),
            );
        }

        Self::parse_trusted_json(log, source, TrustedTsconfigJson::from_parser(json))
    }

    pub(crate) fn parse_trusted_json(
        log: &mut bun_ast::Log,
        source: &bun_ast::Source,
        trusted: TrustedTsconfigJson,
    ) -> Result<Option<Box<TSConfigJSON>>, bun_core::Error> {
        let json = trusted.expr;
                    ...
                    for property in paths.properties.slice() {
                        ...
                        match &value_prop.data {
                            bun_ast::ExprData::EArray(e_array) => {
                                ...
                                if !array.is_empty() {
                                    let mut values = PathTargets::new();
                                    ...
                                    if !values.is_empty() {
                                        ...
                                        let _ = result.paths.put(key, values);
                                    }
