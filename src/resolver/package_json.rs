...
use bun_collections::{ArrayHashMap, MultiArrayList, StringArrayHashMap, smallvec::SmallVec};
use bun_core::Output;
...
impl PackageJSON {
    fn from_machine_read_ref(
        read: crate::dx_machine_cache::PackageJsonMachineReadRef<'_>,
        package_json_path: &[u8],
        contents_static: &'static [u8],
        entry_contents: Box<[u8]>,
        configured_main_fields: &[Box<[u8]>],
        r_fs: &fs::FileSystem,
    ) -> Result<Self, Box<[u8]>> {
        let module_type = match read.module_type() {
            Some(b"commonjs") => ModuleType::Cjs,
            Some(b"module") => ModuleType::Esm,
            Some(_) => return Err(entry_contents),
            None => ModuleType::Unknown,
        };
        let trusted_resolver_payload = read.trusted_resolver_payload();
        let exports = match read.exports() {
            Some(value) => {
                match Self::exports_map_from_machine_value_ref(value, trusted_resolver_payload) {
                    Some(exports) => Some(exports),
                    None => return Err(entry_contents),
                }
            }
            None => None,
        };
        let imports = match read.imports() {
            Some(value) => {
                match Self::exports_map_from_machine_value_ref(value, trusted_resolver_payload) {
                    Some(imports) => Some(imports),
                    None => return Err(entry_contents),
                }
            }
            None => None,
        ...
        let source = bun_ast::Source::init_path_string(package_json_path, contents_static);
        let Some(main_fields) =
            Self::main_fields_from_machine_read_ref(read, configured_main_fields)
        else {
            return Err(entry_contents);
        };
        let browser_map = match read.browser() {
            Some(value) => match Self::browser_map_from_machine_value_ref(value, r_fs) {
                Some(browser_map) => browser_map,
                None => return Err(entry_contents),
            },
            None => BrowserMap::default(),
        };
        let side_effects = match read.side_effects() {
            Some(value) => match Self::side_effects_from_machine_value_ref(value, &source, r_fs) {
                Some(side_effects) => side_effects,
                None => return Err(entry_contents),
            },
            None => SideEffects::Unspecified,
        };
        Ok(PackageJSON {
            name: read.name().map(Box::from).unwrap_or_default(),
            version: read.version().map(Box::from).unwrap_or_default(),
            source: bun_ast::Source {
                path: source.path,
                contents: std::borrow::Cow::Borrowed(contents_static),
                contents_is_recycled: source.contents_is_recycled,
                identifier_name: source.identifier_name,
                index: source.index,
            },
            source_contents: entry_contents,
            module_type,
            exports,
            imports,
            main_fields,
    ...
    fn from_machine_read(
        ...
        let module_type = match read.module_type.as_deref() {
            Some(b"commonjs") => ModuleType::Cjs,
            Some(b"module") => ModuleType::Esm,
            Some(_) => return Err(entry_contents),
            None => ModuleType::Unknown,
        };
        let Some(main_fields) = Self::main_fields_from_machine_read(&read, configured_main_fields)
        else {
            return Err(entry_contents);
        };
        let exports = match read.exports {
            Some(value) => match Self::exports_map_from_machine_value(value) {
                Some(exports) => Some(exports),
                None => return Err(entry_contents),
            },
            None => None,
        ...
        let source = bun_ast::Source::init_path_string(package_json_path, contents_static);
        let browser_map = match read.browser {
            Some(value) => match Self::browser_map_from_machine_value(value, r_fs) {
                Some(browser_map) => browser_map,
                None => return Err(entry_contents),
            },
            None => BrowserMap::default(),
        ...
        let side_effects = match read.side_effects {
            Some(value) => match Self::side_effects_from_machine_value(value, &source, r_fs) {
                Some(side_effects) => side_effects,
                None => return Err(entry_contents),
            },
            None => SideEffects::Unspecified,
        ...
        Ok(PackageJSON {
            ...
            version: read.version.unwrap_or_default(),
            source: bun_ast::Source {
                path: source.path,
                contents: std::borrow::Cow::Borrowed(contents_static),
                contents_is_recycled: source.contents_is_recycled,
    ...
    fn main_fields_from_machine_read_ref(
        read: crate::dx_machine_cache::PackageJsonMachineReadRef<'_>,
        configured_main_fields: &[Box<[u8]>],
    ) -> Option<MainFieldMap> {
        let mut main_fields = MainFieldMap::default();
        for main in configured_main_fields {
            let value = match main.as_ref() {
                b"main" => read.main(),
                b"module" => read.module(),
                b"jsnext:main" => read.jsnext_main(),
                _ => None,
            };
            if let Some(value) = value
                && !value.is_empty()
            {
                main_fields.put(main, Box::from(value)).ok()?;
            }
        }
        Some(main_fields)
    }

    fn main_fields_from_machine_read(
        read: &crate::dx_machine_cache::PackageJsonMachineRead,
        configured_main_fields: &[Box<[u8]>],
    ) -> Option<MainFieldMap> {
        let mut main_fields = MainFieldMap::default();
        for main in configured_main_fields {
            let value = match main.as_ref() {
                b"main" => read.main.as_deref(),
                b"module" => read.module.as_deref(),
                b"jsnext:main" => read.jsnext_main.as_deref(),
                _ => None,
            };
            if let Some(value) = value
                && !value.is_empty()
    ...
    fn browser_map_from_machine_value_ref(
        value: crate::dx_machine_cache::PackageJsonMachineValueRef<'_>,
        r_fs: &fs::FileSystem,
    ) -> Option<BrowserMap> {
        if value.kind() != crate::dx_machine_cache::PackageJsonMachineValueKind::Obj {
            return Some(BrowserMap::default());
        }

        let len = value.object_len()?;
        let mut browser_map = BrowserMap::default();
        browser_map.reserve(len);
        for index in 0..len {
            let (key, value) = value.object_field(index)?;
            let key = FileSystemPackageJsonExt::normalize(r_fs, key);
            match value.kind() {
                crate::dx_machine_cache::PackageJsonMachineValueKind::Str => {
                    browser_map.put(&key, Box::from(value.as_str()?)).ok()?;
                }
                crate::dx_machine_cache::PackageJsonMachineValueKind::Bool => {
                    if !value.as_bool()? {
                        browser_map.put(&key, Box::default()).ok()?;
                    }
                }
                _ => return None,
            }
    ...
    fn browser_map_from_machine_value(
        value: crate::dx_machine_cache::PackageJsonMachineValue,
        r_fs: &fs::FileSystem,
    ) -> Option<BrowserMap> {
        let crate::dx_machine_cache::PackageJsonMachineValue::Obj(fields) = value else {
            return Some(BrowserMap::default());
        };

        let mut browser_map = BrowserMap::default();
        browser_map.reserve(fields.len());
        for (key, value) in fields {
            let key = FileSystemPackageJsonExt::normalize(r_fs, key.as_ref());
            match value {
                crate::dx_machine_cache::PackageJsonMachineValue::Str(value) => {
                    browser_map.put(&key, value).ok()?;
                }
                crate::dx_machine_cache::PackageJsonMachineValue::Bool(false) => {
                    browser_map.put(&key, Box::default()).ok()?;
                }
                crate::dx_machine_cache::PackageJsonMachineValue::Bool(true) => {}
                _ => return None,
            }
        }
        Some(browser_map)
    ...
    fn side_effects_from_machine_value_ref(
        value: crate::dx_machine_cache::PackageJsonMachineValueRef<'_>,
        source: &bun_ast::Source,
        r_fs: &fs::FileSystem,
    ) -> Option<SideEffects> {
        match value.kind() {
            ...
            crate::dx_machine_cache::PackageJsonMachineValueKind::Arr => {
                let len = value.array_len()?;
                let mut items = Vec::with_capacity(len);
                for index in 0..len {
                    let item = value.array_item(index)?;
                    if item.kind() == crate::dx_machine_cache::PackageJsonMachineValueKind::Str {
                        items.push(item.as_str()?);
                    }
                }
                Some(Self::side_effects_from_machine_names(
                    items.iter().copied(),
                    items.len(),
                    source,
                    r_fs,
            ...
            _ => Some(SideEffects::Unspecified),
        }
    }

    fn side_effects_from_machine_value(
        value: crate::dx_machine_cache::PackageJsonMachineValue,
        source: &bun_ast::Source,
        r_fs: &fs::FileSystem,
    ) -> Option<SideEffects> {
        match value {
            crate::dx_machine_cache::PackageJsonMachineValue::Bool(value) => Some(if value {
                SideEffects::Unspecified
            } else {
                SideEffects::False
            }),
            crate::dx_machine_cache::PackageJsonMachineValue::Arr(items) => {
                let mut names = Vec::with_capacity(items.len());
                for item in items {
                    if let crate::dx_machine_cache::PackageJsonMachineValue::Str(name) = item {
                        names.push(name);
                    }
                }
                Some(Self::side_effects_from_machine_names(
    ...
    fn side_effects_from_machine_names<'a>(
        names: impl Iterator<Item = &'a [u8]> + Clone,
        len: usize,
        source: &bun_ast::Source,
        r_fs: &fs::FileSystem,
    ) -> SideEffects {
        ...
        let package_dir = source.path.name().dir_with_trailing_slash();
        if has_globs && has_exact {
            let mut map = SideEffectsMap::default();
            let mut glob_list = GlobList::default();
            map.reserve(len);
            glob_list.reserve(len);

            for name in names {
                if bun_paths::extension(name) == b".css" {
                    continue;
                }
                let joined: [&[u8]; 2] = [package_dir, name];
            ...
            return SideEffects::Mixed(MixedPatterns {
                exact: map,
                globs: glob_list,
            });
        }
        ...
        if has_globs {
            ...
            return SideEffects::Glob(glob_list);
        }
        ...
        for name in names {
            let joined: [&[u8]; 2] = [package_dir, name];
            let _ = map.insert(StringHashMapUnownedKey::init(r_fs.join(&joined)), ());
        }
        SideEffects::Map(map)
    }

    fn exports_map_from_machine_value_ref(
        value: crate::dx_machine_cache::PackageJsonMachineValueRef<'_>,
        trusted_resolver_payload: bool,
    ) -> Option<ExportsMap> {
        if value.kind() == crate::dx_machine_cache::PackageJsonMachineValueKind::Null {
            return None;
        }
        if !trusted_resolver_payload && !Self::machine_value_is_supported_exports_shape(value) {
            return None;
        }
        let expansion_keys = if trusted_resolver_payload {
            &[]
        } else {
            Self::machine_expansion_keys(value)?
        };

        Some(ExportsMap {
            root: Entry {
                first_token: bun_ast::Range::NONE,
                data: EntryData::Machine(MachineEntry {
                    expansion_keys,
                    value: value.into_static(),
                }),
            },
            exports_range: bun_ast::Range::NONE,
            property_key_loc: bun_ast::Loc::EMPTY,
        })
    }

    fn machine_expansion_keys(
        value: crate::dx_machine_cache::PackageJsonMachineValueRef<'_>,
    ) -> Option<&'static [u32]> {
        let Some(len) = value.object_len() else {
            return Some(&[]);
        };
        let mut expansion_keys = SmallVec::<[u32; 8]>::new();
        for index in 0..len {
            let (key, _) = value.object_field_static(index)?;
            if strings::ends_with(key, b"/") || strings::contains_char(key, b'*') {
                expansion_keys.push(u32::try_from(index).expect("package exports map entry index"));
            }
        }
        expansion_keys.sort_by(|a, b| {
            let (left, _) = value
                .object_field_static(*a as usize)
                .expect("validated expansion key index");
            let (right, _) = value
                .object_field_static(*b as usize)
                .expect("validated expansion key index");
            strings::glob_length_compare(left, right)
        });
        ...
        Some(Box::leak(
            Vec::from(expansion_keys.as_slice()).into_boxed_slice(),
        ))
    }

    fn machine_value_is_supported_exports_shape(
        value: crate::dx_machine_cache::PackageJsonMachineValueRef<'_>,
    ) -> bool {
        match value.kind() {
            crate::dx_machine_cache::PackageJsonMachineValueKind::Str
            | crate::dx_machine_cache::PackageJsonMachineValueKind::Bool
            | crate::dx_machine_cache::PackageJsonMachineValueKind::Null => true,
            crate::dx_machine_cache::PackageJsonMachineValueKind::Arr => {
                let Some(len) = value.array_len() else {
                    return false;
                };
                for index in 0..len {
                    let Some(item) = value.array_item(index) else {
                        return false;
                    };
                    if !Self::machine_value_is_supported_exports_shape(item) {
                        return false;
                    }
                }
                true
            }
            crate::dx_machine_cache::PackageJsonMachineValueKind::Obj => {
                let Some(len) = value.object_len() else {
                    return false;
                };
                let mut conditional_sugar: Option<bool> = None;
                for index in 0..len {
                    let Some((key, value)) = value.object_field_static(index) else {
                        return false;
                    };
                    let is_conditional_sugar = !strings::starts_with_char(key, b'.');
                    if let Some(first) = conditional_sugar {
                        if first != is_conditional_sugar {
                            return false;
                        }
                    } else {
                        conditional_sugar = Some(is_conditional_sugar);
                    }
                    if !Self::machine_value_is_supported_exports_shape(value) {
                        return false;
                    }
                }
                true
    ...
    fn exports_map_from_machine_value(
        value: crate::dx_machine_cache::PackageJsonMachineValue,
    ) -> Option<ExportsMap> {
        let root = Self::entry_from_machine_value(value)?;
    ...
    fn entry_from_machine_value_ref(
        value: crate::dx_machine_cache::PackageJsonMachineValueRef<'_>,
    ) -> Option<Entry> {
        let data = match value.kind() {
            crate::dx_machine_cache::PackageJsonMachineValueKind::Str => {
                EntryData::String(EntryString::Source(value.as_static_str()?))
            }
            crate::dx_machine_cache::PackageJsonMachineValueKind::Bool => {
                EntryData::Boolean(value.as_bool()?)
            }
            crate::dx_machine_cache::PackageJsonMachineValueKind::Null => EntryData::Null,
            crate::dx_machine_cache::PackageJsonMachineValueKind::Arr => {
                let len = value.array_len()?;
                let mut output = Vec::with_capacity(len);
                for index in 0..len {
                    output.push(Self::entry_from_machine_value_ref(
                        value.array_item(index)?,
                    )?);
                }
                EntryData::Array(output.into_boxed_slice())
            }
            crate::dx_machine_cache::PackageJsonMachineValueKind::Obj => {
                let len = value.object_len()?;
                ...
                for index in 0..len {
                    let (key, value) = value.object_field_static(index)?;
                    let is_conditional_sugar = !strings::starts_with_char(key, b'.');
                    if let Some(first) = conditional_sugar {
                        if first != is_conditional_sugar {
                            return None;
                    ...
                    if strings::ends_with(key, b"/") || strings::contains_char(key, b'*') {
                        expansion_keys
                            .push(u32::try_from(index).expect("package exports map entry index"));
                    }

                    entries.push(MapEntry {
                        key: EntryString::Source(key),
                        key_range: bun_ast::Range::NONE,
                        value: Self::entry_from_machine_value_ref(value)?,
                    });
                }

                EntryData::Map(EntryDataMap::new(
                    entries,
                    expansion_keys.into_boxed_slice(),
                ))
            }
        };
    ...
    fn entry_from_machine_value(
        value: crate::dx_machine_cache::PackageJsonMachineValue,
    ) -> Option<Entry> {
        let data = match value {
            crate::dx_machine_cache::PackageJsonMachineValue::Str(value) => {
                EntryData::String(EntryString::Owned(value))
            }
            crate::dx_machine_cache::PackageJsonMachineValue::Bool(value) => {
                EntryData::Boolean(value)
            }
            crate::dx_machine_cache::PackageJsonMachineValue::Null => EntryData::Null,
            crate::dx_machine_cache::PackageJsonMachineValue::Arr(items) => {
                let mut output = Vec::with_capacity(items.len());
                for item in items {
                    output.push(Self::entry_from_machine_value(item)?);
                }
                EntryData::Array(output.into_boxed_slice())
            }
            crate::dx_machine_cache::PackageJsonMachineValue::Obj(fields) => {
                ...
                for (index, (key, value)) in fields.into_iter().enumerate() {
                    ...
                    let is_conditional_sugar = !strings::starts_with_char(key_bytes, b'.');
                    if let Some(first) = conditional_sugar {
                        if first != is_conditional_sugar {
                            return None;
                        }
                    ...
                    if strings::ends_with(key_bytes, b"/")
                        || strings::contains_char(key_bytes, b'*')
                    {
                        expansion_keys
                            .push(u32::try_from(index).expect("package exports map entry index"));
                    }
                    ...
                    entries.push(MapEntry {
                        key: EntryString::Owned(key),
                        key_range: bun_ast::Range::NONE,
                        value: Self::entry_from_machine_value(value)?,
                    });
                }
    ...
    pub fn parse<const INCLUDE_DEPENDENCIES: IncludeDependencies>(
        ...
        let package_json_path = r_fs
            ...
            .expect("unreachable");
        crate::dx_machine_cache::record_package_json_machine_cache_proof(
            "parse_attempt",
            package_json_path,
        );
        ...
        if INCLUDE_DEPENDENCIES == IncludeDependencies::None
            && !include_scripts
            && crate::dx_machine_cache::package_json_path_read_enabled()
        {
            let contents_static = b"" as &'static [u8];
            if let Some(package_json) = crate::dx_machine_cache::package_json_read_path_ref_or_warn(
                package_json_path,
                |machine_read| {
                    Self::from_machine_read_ref(
                        machine_read,
                        package_json_path,
                        contents_static,
                        Box::default(),
                ...
                crate::dx_machine_cache::record_package_json_machine_cache_proof(
                    "machine_hit_path_ref",
                    package_json_path,
                );
                return Some(package_json);
            }

            if let Some(machine_read) =
                crate::dx_machine_cache::package_json_read_path_or_warn(package_json_path)
            {
                if let Ok(package_json) = Self::from_machine_read(
                    machine_read,
                    package_json_path,
                    contents_static,
                    Box::default(),
        ...
        let mut entry_contents: Box<[u8]> = match core::mem::take(&mut entry.contents) {
        ...
        if crate::dx_machine_cache::shadow_probe_enabled() {
            let _dx_machine_shadow_probe = crate::dx_machine_cache::shadow_probe_source_or_warn(
                crate::dx_machine_cache::DxMachineCacheKind::PackageJson,
                package_json_path,
                &entry_contents,
            );
        }
        ...
        if INCLUDE_DEPENDENCIES == IncludeDependencies::None
            && !include_scripts
            && crate::dx_machine_cache::read_through_enabled()
        {
            if let Some(machine_read) = crate::dx_machine_cache::package_json_read_source_or_warn(
                package_json_path,
                &entry_contents,
            ) {
                match Self::from_machine_read(
                    machine_read,
                    package_json_path,
                    contents_static,
                    entry_contents,
                    ...
                    Ok(package_json) => return Some(package_json),
                    Err(returned_contents) => {
                        entry_contents = returned_contents;
                    }
                }
            }
        }
            ...
            if let Some(browser_prop) = json.as_property(b"browser") {
                match &browser_prop.expr.data {
                    js_ast::ExprData::EObject(obj) => {
                        ...
                        package_json
                            .browser_map
                            .reserve(obj.properties.len_u32() as usize);
    ...
    pub fn visit(&mut self, expr: js_ast::Expr) -> Entry {
        ...
        match &expr.data {
            ...
            js_ast::ExprData::EString(str) => {
                ...
                // `str.data` is either a source slice or a decoded arena slice.
                debug_assert!(!str.is_utf16);
                return Entry {
                    data: EntryData::String(EntryString::from_parser_slice(
                        self.source,
                        str.data.slice(),
                    )),
            ...
            js_ast::ExprData::EObject(e_obj) => {
                ...
                let mut map_data = EntryDataMapList::with_capacity(prop_len);
                ...
                for (i, prop) in e_obj.properties.slice().iter().enumerate() {
                    ...
                    let key = match prop_key.data.e_string() {
                        Some(s) => EntryString::from_parser_slice(self.source, s.data.slice()),
                        None => EntryString::empty(),
                    };
                    ...
                    let cur_is_conditional_sugar = !strings::starts_with_char(key.as_ref(), b'.');
                    if i == 0 {
                        is_conditional_sugar = cur_is_conditional_sugar;
                    } else if is_conditional_sugar != cur_is_conditional_sugar {
                        let prev_key = map_data.key(i - 1);
                        let prev_key_range = map_data.key_range(i - 1);
                        self.log
                    ...
                    if strings::ends_with(key.as_ref(), b"/")
                        || strings::contains_char(key.as_ref(), b'*')
                    {
                        expansion_keys.push(
                            u32::try_from(map_data.len()).expect("package exports map entry index"),
                        );
                    }
                ...
                return Entry {
                    data: EntryData::Map(EntryDataMap::from_list(
                        map_data,
                        expansion_keys.into_boxed_slice(),
                    )),
                    first_token,
...
pub struct MachineEntry {
    expansion_keys: &'static [u32],
    value: crate::dx_machine_cache::PackageJsonMachineValueRef<'static>,
}

impl MachineEntry {
    #[inline]
    fn kind(self) -> crate::dx_machine_cache::PackageJsonMachineValueKind {
        self.value.kind()
    }

    #[inline]
    fn is_map(self) -> bool {
        self.kind() == crate::dx_machine_cache::PackageJsonMachineValueKind::Obj
    }

    fn keys_start_with_dot(self) -> bool {
        let Some(len) = self.value.object_len() else {
            return false;
        };
        if len == 0 {
            return false;
        ...
        self.value
            .object_field_static(0)
            .map(|(key, _)| strings::starts_with_char(key, b'.'))
            .unwrap_or(false)
    }
    ...
    fn value_for_key(
        self,
        key: &[u8],
    ) -> Option<crate::dx_machine_cache::PackageJsonMachineValueRef<'static>> {
        self.value.object_value_for_key_static(key)
    }
    ...
    fn field_at(
        ...
        &'static [u8],
        crate::dx_machine_cache::PackageJsonMachineValueRef<'static>,
    )> {
        let (key, value) = self.value.object_field_static(index)?;
        Some((key, value.into_static()))
    }

    #[inline]
    fn expansion_keys(self) -> &'static [u32] {
        self.expansion_keys
    }

    fn sorted_expansion_indices(self) -> Option<SmallVec<[u32; 8]>> {
        let len = self.value.object_len()?;
        let mut expansion_keys = SmallVec::<[u32; 8]>::new();
        for index in 0..len {
            let (key, _) = self.value.object_field_static(index)?;
            if strings::ends_with(key, b"/") || strings::contains_char(key, b'*') {
                expansion_keys.push(u32::try_from(index).expect("package exports map entry index"));
            }
        }
...
pub struct EntryDataMap {
    ...
    // Sorted indices into `list` for wildcard or folder export/import expansion keys.
    pub expansion_keys: Box<[u32]>,
    // Sorted indices into `list` for value_for_key when the map is large enough
    // for an index to beat a tiny linear scan. Empty means use `list` directly.
    pub exact_keys: Box<[u32]>,
    pub list: EntryDataMapList,
}

const EXPORTS_EXACT_KEY_INDEX_MIN_ENTRIES: usize = 128;

impl EntryDataMap {
    pub fn new(entries: Vec<MapEntry>, expansion_keys: Box<[u32]>) -> Self {
        let mut list = EntryDataMapList::with_capacity(entries.len());
        for entry in entries {
            list.push(entry);
        }
        Self::from_list(list, expansion_keys)
    }

    fn from_list(list: EntryDataMapList, mut expansion_keys: Box<[u32]>) -> Self {
        Self::sorted_expansion_key_indices(&list, &mut expansion_keys);
        let exact_keys = if list.len() >= EXPORTS_EXACT_KEY_INDEX_MIN_ENTRIES
            && !dx_disable_exports_exact_key_index()
        {
            Self::sorted_key_indices(&list)
        } else {
            Box::default()
        };
        Self {
    ...
    fn sorted_expansion_key_indices(list: &EntryDataMapList, expansion_keys: &mut [u32]) {
        expansion_keys.sort_by(|a, b| {
            strings::glob_length_compare(
                list.key(*a as usize).as_ref(),
                list.key(*b as usize).as_ref(),
            )
        });
    }

    fn sorted_key_indices(list: &EntryDataMapList) -> Box<[u32]> {
        let mut exact_keys: Vec<u32> = (0..list.len())
            .map(|i| u32::try_from(i).expect("package exports map entry index"))
            .collect();
        exact_keys.sort_unstable_by(|a, b| {
            list.key(*a as usize)
                .as_ref()
                .cmp(list.key(*b as usize).as_ref())
        });
        exact_keys.into_boxed_slice()
    }

    pub fn value_for_key(&self, key: &[u8]) -> Option<&Entry> {
        if self.exact_keys.is_empty() {
        ...
        let start = self
            .exact_keys
            .partition_point(|entry_i| self.list.key(*entry_i as usize).as_ref() < key);

        let mut best: Option<u32> = None;
        for entry_i in &self.exact_keys[start..] {
            if self.list.key(*entry_i as usize).as_ref() != key {
                break;
            }
            best = Some(best.map_or(*entry_i, |prev| prev.min(*entry_i)));
        }

        best.map(|entry_i| self.list.value(entry_i as usize))
    }
}

#[inline]
fn dx_disable_exports_exact_key_index() -> bool {
    static DISABLE: std::sync::LazyLock<bool> = std::sync::LazyLock::new(|| {
        std::env::var_os("BUN_DX_DISABLE_EXPORTS_EXACT_KEY_INDEX").is_some()
    });
    *DISABLE
}

#[inline]
fn dx_disable_resolver_inline_conditions_map() -> bool {
    static DISABLE: std::sync::LazyLock<bool> = std::sync::LazyLock::new(|| {
        std::env::var_os("BUN_DX_DISABLE_RESOLVER_INLINE_CONDITIONS_MAP").is_some()
    });
    *DISABLE
}

pub struct EntryDataMapList {
    entries: MultiArrayList<MapEntry>,
}

impl EntryDataMapList {
    fn with_capacity(capacity: usize) -> Self {
        let mut entries = MultiArrayList::default();
        entries.ensure_total_capacity(capacity).expect("OOM");
        Self { entries }
    }

    fn push(&mut self, entry: MapEntry) {
        self.entries.push(entry).expect("OOM");
    }
    ...
    fn key(&self, index: usize) -> &EntryString {
        &self.entries.items::<"key", EntryString>()[index]
    }

    fn key_range(&self, index: usize) -> bun_ast::Range {
        self.entries.items::<"key_range", bun_ast::Range>()[index]
    }

    fn value(&self, index: usize) -> &Entry {
        &self.entries.items::<"value", Entry>()[index]
    }

    fn iter(&self) -> EntryDataMapListIter<'_> {
        EntryDataMapListIter {
            keys: self.entries.items::<"key", EntryString>(),
            key_ranges: self.entries.items::<"key_range", bun_ast::Range>(),
            values: self.entries.items::<"value", Entry>(),
            index: 0,
        }
    }
}

impl Clone for EntryDataMapList {
    fn clone(&self) -> Self {
        let mut cloned = EntryDataMapList::with_capacity(self.len());
        for entry in self.iter() {
            cloned.push(MapEntry {
                key: (*entry.key).clone(),
...
impl Drop for EntryDataMapList {
    fn drop(&mut self) {
...
pub struct MapEntryRef<'a> {
    pub key: &'a EntryString,
    pub key_range: bun_ast::Range,
    pub value: &'a Entry,
}

pub struct EntryDataMapListIter<'a> {
    keys: &'a [EntryString],
    key_ranges: &'a [bun_ast::Range],
    values: &'a [Entry],
    index: usize,
}

impl<'a> Iterator for EntryDataMapListIter<'a> {
    type Item = MapEntryRef<'a>;
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        Some(MapEntryRef {
            key: &self.keys[index],
            key_range: self.key_ranges[index],
...
impl EntryString {
    ...
    #[cfg(test)]
    fn owned(value: &[u8]) -> Self {
        if value.is_empty() {
            Self::Empty
        } else {
            Self::Owned(Box::from(value))
        }
    }

    fn from_parser_slice(source: &bun_ast::Source, value: &[u8]) -> Self {
        if value.is_empty() {
            return Self::Empty;
        }

        if let std::borrow::Cow::Borrowed(contents) = &source.contents {
            if let Some(value) = source_subslice(*contents, value) {
...
impl core::ops::Deref for EntryString {
    type Target = [u8];
...
fn source_subslice(source: &'static [u8], value: &[u8]) -> Option<&'static [u8]> {
    let source_start = source.as_ptr() as usize;
    let source_end = source_start.checked_add(source.len())?;
    let value_start = value.as_ptr() as usize;
    let value_end = value_start.checked_add(value.len())?;

    if value_start < source_start || value_end > source_end {
        return None;
    }

    let offset = value_start - source_start;
...
impl Entry {
    pub fn keys_start_with_dot(&self) -> bool {
        match &self.data {
            EntryData::Map(m) => {
                !m.list.is_empty() && strings::starts_with_char(m.list.key(0).as_ref(), b'.')
            }
            EntryData::Machine(machine) => machine.keys_start_with_dot(),
            _ => false,
        }
    }
    ...
    pub fn is_map_like(&self) -> bool {
        match &self.data {
            EntryData::Map(_) => true,
            EntryData::Machine(machine) => machine.is_map(),
            _ => false,
        }
    }
    ...
    pub fn value_for_key(&self, key_: &[u8]) -> Option<&Entry> {
        match &self.data {
            EntryData::Map(m) => m.value_for_key(key_),
            _ => None,
        }
    }
}
...
mod tests {
    ...
    fn string_entry(value: &[u8]) -> Entry {
        Entry {
            first_token: bun_ast::Range::NONE,
            data: EntryData::String(EntryString::owned(value)),
        }
    }

    fn map_entry(key: &[u8], value: &[u8]) -> MapEntry {
        MapEntry {
            key: EntryString::owned(key),
            key_range: bun_ast::Range::NONE,
    ...
    fn string_value(entry: &Entry) -> &[u8] {
        match &entry.data {
            EntryData::String(value) => value.as_ref(),
            _ => unreachable!("test entries are strings"),
        }
    }

    #[test]
    fn exact_key_index_preserves_value_for_key_semantics() {
        let entry = Entry {
            first_token: bun_ast::Range::NONE,
            data: EntryData::Map(EntryDataMap::new(
                vec![
                    map_entry(b"./dup", b"./first.js"),
                    map_entry(b"./feature/*", b"./feature/*.js"),
                    map_entry(b"./dup", b"./second.js"),
                    map_entry(b"./folder/", b"./folder/"),
                    map_entry(b"./other", b"./other.js"),
                ],
                Box::default(),
            )),
        };
        ...
        assert_eq!(
            string_value(entry.value_for_key(b"./dup").unwrap()),
            b"./first.js"
        );
        assert_eq!(
            string_value(entry.value_for_key(b"./feature/*").unwrap()),
            b"./feature/*.js"
        );
        assert_eq!(
            string_value(entry.value_for_key(b"./folder/").unwrap()),
            b"./folder/"
        );
        assert!(entry.value_for_key(b"./missing").is_none());
    }

    #[test]
    fn exact_key_index_builds_for_large_maps_and_preserves_first_duplicate() {
        let mut entries = Vec::new();
        entries.push(map_entry(b"./dup", b"./first.js"));
        for i in 0..EXPORTS_EXACT_KEY_INDEX_MIN_ENTRIES {
            let key = format!("./item-{i}");
            let value = format!("./item-{i}.js");
            entries.push(map_entry(key.as_bytes(), value.as_bytes()));
        }
        entries.push(map_entry(b"./dup", b"./second.js"));

        let map = EntryDataMap::new(entries, Box::default());
        assert!(!map.exact_keys.is_empty());
        assert_eq!(
            string_value(map.value_for_key(b"./dup").unwrap()),
            b"./first.js"
        );
    }

    #[test]
    fn expansion_key_indices_preserve_sorted_key_value_pairs() {
        let map = EntryDataMap::new(
            vec![
                map_entry(b"./feature/*", b"./general/*.js"),
                map_entry(b"./feature/special/*", b"./special/*.js"),
                map_entry(b"./feature/*/deep", b"./deep/*.js"),
            ],
            vec![0, 1, 2].into_boxed_slice(),
        );

        let sorted_pairs: Vec<(&[u8], &[u8])> = map
            .expansion_keys
            .iter()
            .map(|entry_i| {
                let entry_i = *entry_i as usize;
                (
                    map.list.key(entry_i).as_ref(),
                    string_value(map.list.value(entry_i)),
                )
            })
            .collect();
        ...
        assert_eq!(
            sorted_pairs,
            vec![
                (&b"./feature/special/*"[..], &b"./special/*.js"[..]),
                (&b"./feature/*/deep"[..], &b"./deep/*.js"[..]),
                (&b"./feature/*"[..], &b"./general/*.js"[..]),
            ]
        );
    }

    #[test]
    fn entry_string_only_borrows_source_subslices() {
        let contents: &'static [u8] = br#"{"exports":{"./x":"./x.js"}}"#;
        let source = bun_ast::Source::init_path_string("package.json", contents);
        let source_value = EntryString::from_parser_slice(&source, &contents[13..16]);
        let external = EntryString::from_parser_slice(&source, b"./x");

        assert!(matches!(source_value, EntryString::Source(value) if value == b"./x"));
        assert!(matches!(external, EntryString::Owned(value) if value.as_ref() == b"./x"));
    }

    #[test]
    fn conditions_map_promotes_without_losing_duplicate_or_clone_semantics() {
        let mut conditions = ConditionsMap::default();
        for i in 0..INLINE_CONDITIONS_CAPACITY {
            let key = format!("condition-{i}");
            assert_eq!(conditions.insert(key.as_bytes(), ()), None);
        }

        assert_eq!(conditions.insert(b"condition-3", ()), Some(()));
        assert!(conditions.map.is_none());
        assert_eq!(conditions.insert(b"condition-overflow", ()), None);
        assert!(conditions.map.is_some());
        assert!(conditions.contains(b"condition-3"));
        assert!(conditions.contains_key(b"condition-overflow"));
        assert!(!conditions.contains_key(b"no-active-condition-with-this-length"));

        let cloned = conditions.clone().expect("clone conditions");
        assert!(cloned.contains(b"condition-3"));
        assert!(cloned.contains(b"condition-overflow"));
        assert!(!cloned.contains_key(b"no-active-condition-with-this-length"));
    }
}

const INLINE_CONDITIONS_CAPACITY: usize = 12;
const CONDITIONS_LENGTH_MASK_BITS: usize = u128::BITS as usize;

#[derive(Default)]
pub struct ConditionsMap {
    inline: SmallVec<[Box<[u8]>; INLINE_CONDITIONS_CAPACITY]>,
    map: Option<StringArrayHashMap<()>>,
    length_mask: u128,
    has_long_condition: bool,
}
...
impl ConditionsMap {
    pub fn reserve(&mut self, additional: usize) {
        ...
        if dx_disable_resolver_inline_conditions_map() {
            self.promote(additional);
            return;
        }

        if self.inline.len() + additional > INLINE_CONDITIONS_CAPACITY {
            self.promote(additional);
        }
    }

    pub fn insert(&mut self, key: &[u8], value: ()) -> Option<()> {
        if self.map.is_some() {
            self.remember_key_len(key);
            return self.map.as_mut().and_then(|map| map.insert(key, value));
        }
        ...
        if self
            .inline
            .iter()
            .any(|condition| condition.as_ref() == key)
        {
            return Some(());
        }

        if self.inline.len() < INLINE_CONDITIONS_CAPACITY {
            self.remember_key_len(key);
            self.inline.push(Box::from(key));
            return None;
        }

        self.promote(1);
        self.remember_key_len(key);
        self.map.as_mut().and_then(|map| map.insert(key, value))
    }

    #[inline]
    pub fn contains(&self, key: &[u8]) -> bool {
        if !self.may_contain_key_len(key.len()) {
            return false;
        }
    ...
    pub fn clone(&self) -> Result<Self, bun_alloc::AllocError> {
        Ok(Self {
            ...
            has_long_condition: self.has_long_condition,
        })
    }

    fn promote(&mut self, additional: usize) {
        let mut map = StringArrayHashMap::<()>::with_capacity(self.inline.len() + additional);
        for condition in self.inline.drain(..) {
            map.insert(condition.as_ref(), ());
        }
        self.map = Some(map);
    }

    #[inline]
    fn remember_key_len(&mut self, key: &[u8]) {
        if key.len() < CONDITIONS_LENGTH_MASK_BITS {
            self.length_mask |= 1u128 << key.len();
        } else {
            self.has_long_condition = true;
        }
    }

    #[inline]
    fn may_contain_key_len(&self, len: usize) -> bool {
        if len < CONDITIONS_LENGTH_MASK_BITS {
            return (self.length_mask & (1u128 << len)) != 0;
        }

        self.has_long_condition
    }
}
...
pub struct Resolution {
    ...
    // buffer. Rust still copies out because `EntryData::String` may own decoded
    // arena data and the threadlocal path buffers are reused.
    pub path: Box<[u8]>,
...
pub struct ResolutionDebug {
    ...
    pub unmatched_conditions: SmallVec<[Box<[u8]>; 4]>,
}
...
pub struct ReverseResolution {
    ...
    // TODO(perf): avoid copying when the result already points at source-backed
    // `EntryString` data with a package-json lifetime.
    pub subpath: Box<[u8]>,
    pub token: bun_ast::Range,
}
...
impl<'a> ESModule<'a> {
    ...
    pub fn finalize(result_: Resolution) -> Resolution {
        ...
        if strings::index_of_char(&result.path, b'%').is_none() {
    ...
    fn resolve_exports(
        ...
        if let EntryData::Machine(machine) = &exports.data {
            return self.resolve_machine_exports(
                package_url,
                subpath,
                *machine,
                exports.first_token,
        ...
        } else if exports.is_map_like() && exports.keys_start_with_dot() {
    ...
    fn resolve_machine_exports(
        &mut self,
        package_url: &[u8],
        subpath: &[u8],
        exports: MachineEntry,
        token: bun_ast::Range,
    ) -> Resolution {
        if subpath == b"." {
            match exports.kind() {
                crate::dx_machine_cache::PackageJsonMachineValueKind::Str
                | crate::dx_machine_cache::PackageJsonMachineValueKind::Arr => {
                    let result = self.resolve_target_machine::<false>(
                        package_url,
                        exports,
                        b"",
                        false,
                ...
                crate::dx_machine_cache::PackageJsonMachineValueKind::Obj
                    if !exports.keys_start_with_dot() =>
                {
                    let result = self.resolve_target_machine::<false>(
                        package_url,
                        exports,
                ...
                crate::dx_machine_cache::PackageJsonMachineValueKind::Obj => {
                    if let Some(main_value) = exports.value_for_key(b".") {
                        if let Some(result) = self.resolve_simple_machine_string_target(
                            package_url,
                            main_value,
                            b"",
                            false,
                        ...
                        let result = self.resolve_target_machine_value::<false>(
                            package_url,
                            main_value,
                            b"",
                            false,
        ...
        } else if exports.is_map() && exports.keys_start_with_dot() {
            let result =
                self.resolve_machine_imports_exports(subpath, exports, false, package_url, token);
            if result.status != Status::Null && result.status != Status::Undefined {
                return result;
            }
    ...
    fn resolve_imports_exports(
        ...
        if let EntryData::Machine(machine) = &match_obj.data {
            return self.resolve_machine_imports_exports(
                match_key,
                *machine,
                is_imports,
                package_url,
                match_obj.first_token,
            );
        }
        ...
        if let EntryData::Map(map) = &match_obj.data {
            ...
            for expansion_i in expansion_keys.iter() {
                let expansion_i = *expansion_i as usize;
                let expansion_key = map.list.key(expansion_i).as_ref();
                // If expansionKey contains "*", set patternBase to the substring of
                // expansionKey up to but excluding the first "*" character
                if let Some(star) = strings::index_of_char(expansion_key, b'*') {
                    let star = star as usize;
                    let pattern_base = &expansion_key[0..star];
                    ...
                    if strings::starts_with(match_key, pattern_base) {
                        ...
                        let pattern_trailer = &expansion_key[star + 1..];
                        ...
                        {
                            let target = map.list.value(expansion_i);
                    ...
                    // expansionKey, then
                    if strings::starts_with(match_key, expansion_key) {
                        let target = map.list.value(expansion_i);
                        let subpath = &match_key[expansion_key.len()..];
                        if let Some(log) = self.debug_logs.as_deref_mut() {
                            log.add_note_fmt(format_args!(
                                "The key \"{}\" matched with \"{}\" left over",
                                bstr::BStr::new(expansion_key),
    ...
    fn resolve_machine_imports_exports(
        &mut self,
        match_key: &[u8],
        match_obj: MachineEntry,
        is_imports: bool,
        package_url: &[u8],
        token: bun_ast::Range,
    ) -> Resolution {
        ...
        if !strings::ends_with_char(match_key, b'/') && !strings::contains_char(match_key, b'*') {
            if let Some(target) = match_obj.value_for_key(match_key) {
                ...
                return self.resolve_target_machine_value::<false>(
                    package_url,
                    target,
                    b"",
                    is_imports,
        ...
        let expansion_keys_storage;
        let expansion_keys = if !match_obj.expansion_keys().is_empty() {
            match_obj.expansion_keys()
        } else {
            let Some(computed) = match_obj.sorted_expansion_indices() else {
                return Resolution {
                    status: Status::InvalidPackageConfiguration,
                    debug: ResolutionDebug {
                        token,
            ...
            expansion_keys_storage = computed;
            expansion_keys_storage.as_slice()
        };

        for expansion_i in expansion_keys.iter() {
            let expansion_i = *expansion_i as usize;
            let Some((expansion_key, target)) = match_obj.field_at(expansion_i) else {
                return Resolution {
                    status: Status::InvalidPackageConfiguration,
                    debug: ResolutionDebug {
                        token,
            ...
            if let Some(star) = strings::index_of_char(expansion_key, b'*') {
                ...
                if strings::starts_with(match_key, pattern_base) {
                            ...
                            && match_key.len() >= expansion_key.len())
                    {
                        ...
                        return self.resolve_target_machine_value::<true>(
                            package_url,
                            target,
                            subpath,
                            is_imports,
            ...
            } else if strings::starts_with(match_key, expansion_key) {
                let subpath = &match_key[expansion_key.len()..];
                if let Some(log) = self.debug_logs.as_deref_mut() {
                    log.add_note_fmt(format_args!(
                        "The key \"{}\" matched with \"{}\" left over",
                ...
                let mut result = self.resolve_target_machine_value::<false>(
                    package_url,
                    target,
                    subpath,
                    is_imports,
    ...
    fn resolve_target_machine<const PATTERN: bool>(
        &mut self,
        package_url: &[u8],
        target: MachineEntry,
        subpath: &[u8],
        ...
        self.resolve_target_machine_value::<PATTERN>(
            package_url,
            target.value,
            subpath,
            internal,
    ...
    fn resolve_target_machine_value<const PATTERN: bool>(
        &mut self,
        package_url: &[u8],
        target: crate::dx_machine_cache::PackageJsonMachineValueRef<'static>,
        subpath: &[u8],
        internal: bool,
        token: bun_ast::Range,
    ) -> Resolution {
        if self.debug_logs.is_some() {
            return self.resolve_target_machine_value_with_entry::<PATTERN>(
                package_url,
                target,
                subpath,
                internal,
        ...
        match target.kind() {
            crate::dx_machine_cache::PackageJsonMachineValueKind::Str => {
                let result = if PATTERN {
                    self.resolve_simple_machine_string_target_pattern(
                        package_url,
                        target,
                        subpath,
                        internal,
                    ...
                    self.resolve_simple_machine_string_target(
                        package_url,
                        target,
                        subpath,
                        internal,
                ...
                result.unwrap_or_else(|| {
                    self.resolve_target_machine_value_with_entry::<PATTERN>(
                        package_url,
                        target,
                        subpath,
                        internal,
            ...
            crate::dx_machine_cache::PackageJsonMachineValueKind::Obj => {
                let Some(len) = target.object_len() else {
                    return invalid_package_configuration(token);
                };
                for index in 0..len {
                    let Some((key, value)) = target.object_field_static(index) else {
                        return invalid_package_configuration(token);
                    };
                    if self.conditions.contains_key(key) {
                        let prev_module_type = *self.module_type;
                        let result = self.resolve_target_machine_value::<PATTERN>(
                            package_url,
                            value.into_static(),
                            subpath,
                            internal,
            ...
            crate::dx_machine_cache::PackageJsonMachineValueKind::Arr => {
                ...
                for index in 0..len {
                    let Some(target_value) = target.array_item(index) else {
                        return invalid_package_configuration(token);
                    };
                    let prev_module_type = *self.module_type;
                    let result = self.resolve_target_machine_value::<PATTERN>(
                        package_url,
                        target_value.into_static(),
                        subpath,
                        internal,
                        token,
                    );
            ...
            crate::dx_machine_cache::PackageJsonMachineValueKind::Null => Resolution {
                path: Box::default(),
                status: Status::Null,
                debug: ResolutionDebug {
                    token,
            ...
            crate::dx_machine_cache::PackageJsonMachineValueKind::Bool => Resolution {
    ...
    fn resolve_target_machine_value_with_entry<const PATTERN: bool>(
        &mut self,
        package_url: &[u8],
        target: crate::dx_machine_cache::PackageJsonMachineValueRef<'static>,
        subpath: &[u8],
        ...
        let Some(target) = PackageJSON::entry_from_machine_value_ref(target) else {
            return invalid_package_configuration(token);
        };
        self.resolve_target::<PATTERN>(package_url, &target, subpath, internal)
    }

    fn resolve_simple_machine_string_target(
        &mut self,
        package_url: &[u8],
        target: crate::dx_machine_cache::PackageJsonMachineValueRef<'static>,
        subpath: &[u8],
        ...
        self.resolve_simple_machine_string_target_impl::<false>(
            package_url,
            target,
            subpath,
            internal,
    ...
    fn resolve_simple_machine_string_target_pattern(
        &mut self,
        package_url: &[u8],
        target: crate::dx_machine_cache::PackageJsonMachineValueRef<'static>,
        subpath: &[u8],
        ...
        self.resolve_simple_machine_string_target_impl::<true>(
            package_url,
            target,
            subpath,
            internal,
    ...
    fn resolve_simple_machine_string_target_impl<const PATTERN: bool>(
        ...
        if internal || self.debug_logs.is_some() {
            return None;
        }
        let str = target.as_static_str()?;
        ...
        if !strings::starts_with(str, b"./") || find_invalid_segment(str).is_some() {
            return None;
        }
        ...
        if find_invalid_segment(resolved_target).is_some() {
            return None;
        }

        let (result, status) = if PATTERN {
            ...
            if find_invalid_segment(result).is_some() {
                return None;
            }
            let status = if strings::ends_with_char(result, b'*')
                && strings::index_of_char(result, b'*')
                    .is_some_and(|index| index as usize == result.len() - 1)
            {
            ...
            let result = resolve_path::resolve_path::join_string_buf::<resolve_path::platform::Auto>(
                &mut resolve_target_buf2.0,
                &parts2,
            );
            (result, Status::Exact)
        };
    ...
    fn resolve_target<const PATTERN: bool>(
        ...
        match &target.data {
            ...
            EntryData::Machine(machine) => {
                return self.resolve_target_machine::<PATTERN>(
                    package_url,
                    *machine,
                    subpath,
                    internal,
            ...
            EntryData::Map(object) => {
                ...
                if !object.list.is_empty() && !target.keys_start_with_dot() {
                    let last_map_entry_value = object.list.value(last_map_entry_i);
                    ...
                    let unmatched: SmallVec<[Box<[u8]>; 4]> = match &return_target.data {
                        EntryData::Map(m) => {
                            m.list.iter().map(|e| e.key.to_boxed_slice()).collect()
                        }
                        _ => SmallVec::new(),
                    };
...
fn invalid_package_configuration(token: bun_ast::Range) -> Resolution {
    Resolution {
        status: Status::InvalidPackageConfiguration,
        debug: ResolutionDebug {
            token,
