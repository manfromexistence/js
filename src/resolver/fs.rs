...
const PREHASHED_PACKAGE_JSON_QUERY: u64 = 0x1390_4afe_e78a_d89e;
const PREHASHED_NODE_MODULES_QUERY: u64 = 0xaa75_77ab_5f82_52c4;
const PREHASHED_DOT_BIN_QUERY: u64 = 0xe803_5301_98be_9dbd;
const PREHASHED_TSCONFIG_JSON_QUERY: u64 = 0xbdfd_b2d7_9ce9_5339;
const PREHASHED_JSCONFIG_JSON_QUERY: u64 = 0xe904_6448_621c_fd7a;

#[inline(always)]
fn prehashed_literal_query(query_lower: &'static [u8]) -> Option<u64> {
    match query_lower {
        b"package.json" => Some(PREHASHED_PACKAGE_JSON_QUERY),
        b"node_modules" => Some(PREHASHED_NODE_MODULES_QUERY),
        b".bin" => Some(PREHASHED_DOT_BIN_QUERY),
        b"tsconfig.json" => Some(PREHASHED_TSCONFIG_JSON_QUERY),
        b"jsconfig.json" => Some(PREHASHED_JSCONFIG_JSON_QUERY),
        _ => None,
    }
}
...
impl DirEntry {
    ...
    // PORT NOTE: Rust callers pass an already-lowercase literal. Keep the hot
    // resolver probes on the Zig-style prehashed path where the map hash is a
    // literal; fallback callers still derive the hash from this map's hasher.
    pub fn get_comptime_query<'a>(&'a self, query_lower: &'static [u8]) -> Option<EntryLookup<'a>> {
        let query_hash =
            prehashed_literal_query(query_lower).unwrap_or_else(|| self.data.hash_key(query_lower));
        debug_assert_eq!(query_hash, self.data.hash_key(query_lower));

        let &result_ptr = self.data.get_hashed(query_hash, query_lower)?;
    ...
    pub fn has_comptime_query(&self, query_lower: &'static [u8]) -> bool {
        ...
        self.data.get_hashed(query_hash, query_lower).is_some()
    }
...
mod tests {
    ...
    #[test]
    fn prehashed_literal_query_hashes_match_dir_entry_map_hasher() {
        let map = dir_entry::EntryMap::default();
        ...
        for query in [
            ...
            assert_eq!(prehashed_literal_query(query), Some(map.hash_key(query)));
        }
    }
}
