...
use crate::tsconfig_json::{PathTargets, TSConfigJSON};
...
#[inline]
fn dx_disable_tsconfig_precomputed_path_matcher() -> bool {
    static DISABLE: std::sync::LazyLock<bool> = std::sync::LazyLock::new(|| {
        std::env::var_os("BUN_DX_DISABLE_TSCONFIG_PRECOMPUTED_PATH_MATCHER").is_some()
    });
    *DISABLE
}
...
impl<'a> Resolver<'a> {
    ...
    pub fn match_tsconfig_paths(
        ...
        let can_match_exact_path = tsconfig.exact_path_max_len > 0
            && path_len >= tsconfig.exact_path_min_len
            && path_len <= tsconfig.exact_path_max_len;
        if can_match_exact_path && let Some(value) = tsconfig.paths.get(path) {
        ...
        if dx_disable_tsconfig_precomputed_path_matcher() {
            return self.match_tsconfig_paths_without_precomputed_patterns(
                tsconfig,
                path,
                kind,
                out,
        ...
        if let Some(longest_match) = longest_match {
            ...
            for target in longest_match.targets.iter() {
                let prefix_parts: [&[u8]; 2] = [abs_base_url, &target.prefix];
                ...
                if !target.has_wildcard {
                    if self
                        .load_as_file_or_directory(prefix, kind, out)
                        .is_success()
                    {
                        return MatchStatus::Success;
                    }
                ...
                let matched_text_segment = if target.suffix.is_empty() {
                    matched_text
                } else {
                    let matched_text_with_suffix_len = matched_text.len() + target.suffix.len();
                    if matched_text_with_suffix_len > matched_text_with_suffix.len() {
                        continue;
                    }
                    ::bun_core::concat_into(
                        matched_text_with_suffix,
                        &[matched_text, &target.suffix],
                    );
                ...
                let parts: [&[u8]; 2] = [prefix, strings::trim_left(matched_text_segment, b"/")];
    ...
    fn match_tsconfig_paths_without_precomputed_patterns(
        &mut self,
        tsconfig: &TSConfigJSON,
        path: &[u8],
        kind: ast::ImportKind,
        ...
        let mut longest_match: Option<(&[u8], &[u8], &PathTargets)> = None;
        for (key, value) in tsconfig.paths.iter() {
            let key = key.as_ref();
            let Some(star) = strings::index_of_char(key, b'*') else {
                continue;
            };
            ...
            let replace = longest_match
                .map(|(best_prefix, best_suffix, _)| {
                    prefix.len() > best_prefix.len()
                        || (prefix.len() == best_prefix.len() && suffix.len() > best_suffix.len())
                })
                .unwrap_or(true);
            if replace {
                longest_match = Some((prefix, suffix, value));
            }
        }

        let Some((prefix, suffix, targets)) = longest_match else {
            return MatchStatus::NotFound;
        };
        ...
        for original_path in targets.iter() {
            let matched_text = &path[prefix.len()..path.len() - suffix.len()];
            let Some(star) = strings::index_of_char(original_path, b'*') else {
                let prefix_parts: [&[u8]; 2] = [abs_base_url, original_path.as_ref()];
                let Some(absolute_original_path) = self
                    .fs_ref()
                    .abs_buf_checked(&prefix_parts, bufs!(tsconfig_match_full_buf))
                else {
                    continue;
                };
            ...
            let star = star as usize;
            let target_prefix = &original_path[..star];
            let target_suffix = strings::trim_left(&original_path[star..], b"*");

            let matched_text_with_suffix = bufs!(tsconfig_match_full_buf3);
            let matched_text_segment = if target_suffix.is_empty() {
                matched_text
            } else {
                let matched_text_with_suffix_len = matched_text.len() + target_suffix.len();
                if matched_text_with_suffix_len > matched_text_with_suffix.len() {
                    continue;
                }
                ::bun_core::concat_into(matched_text_with_suffix, &[matched_text, target_suffix]);
                &matched_text_with_suffix[0..matched_text_with_suffix_len]
            };

            let prefix_parts: [&[u8]; 2] = [abs_base_url, target_prefix];
            let Some(prefix) = self
                .fs_ref()
                .abs_buf_checked(&prefix_parts, bufs!(tsconfig_match_full_buf2))
            else {
                ...
                if let Some(tsconfig_json) = parsed_tsconfig {
                    ...
                    while let Some(parent_config_ptr) = parent_configs.pop() {
                        ...
                        if !parent_config.base_url_for_paths.is_empty() {
                            ...
                            mc.path_patterns = core::mem::take(&mut parent_config.path_patterns);
                            mc.exact_path_min_len = parent_config.exact_path_min_len;
                            mc.exact_path_max_len = parent_config.exact_path_max_len;
