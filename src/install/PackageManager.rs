...
use bun_collections::{
    ArrayHashMap, HashMap, HiveArrayFallback, IdentityContext, LinearFifo, StringArrayHashMap, U64,
};
use bun_core::ZBox;
...
pub(crate) type TaskDependencyQueue =
    HashMap<Task::Id, TaskCallbackList, IdentityContext<Task::Id>>;
...
type RepositoryMap = HashMap<Task::Id, Fd, IdentityContext<Task::Id>>;
/// Zig: `FolderResolution.Map` (resolvers/folder_resolver.zig) =
/// `std.HashMap(u64, FolderResolution, IdentityContext(u64), 80)`.
pub(crate) type FolderResolutionMap = HashMap<u64, FolderResolution, IdentityContext<u64>>;
pub(crate) type NpmAliasMap =
    HashMap<PackageNameHash, crate::dependency::Version, IdentityContext<PackageNameHash>>;
...
pub struct PackageManager {
    ...
    pub patched_dependencies_to_remove: ArrayHashMap<PackageNameAndVersionHash, (), U64>,
...
pub fn init(
    ...
    unsafe {
        ...
        wr!(thread_pool, {
            let mut pool = ThreadPool::init(thread_pool::Config {
                max_threads: cpu_count,
                ..Default::default()
            });
            pool.needs_stack_bounds = false;
            pool
        });
