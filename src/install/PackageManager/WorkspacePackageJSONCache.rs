...
impl WorkspacePackageJSONCache {
    ...
    pub fn get_with_path(
        ...
        let path_hash = self.map.hash_key(path);
        match self.map.get_or_try_insert_hashed(path_hash, path, || {
            ...
            let source = bun_ast::to_source(&key, Default::default())
                .map_err(|err| GetMissError::Read(err.into()))?;
            ...
            let json_bump = bun_alloc::Arena::new();
            let parsed = parse_package_json(&source, log, &json_bump, opts.guess_indentation)
                .map_err(GetMissError::Parse)?;
            ...
            Ok(entry) => GetResult::Entry(entry),
            Err(GetMissError::Read(err)) => GetResult::ReadErr(err),
            Err(GetMissError::Parse(err)) => GetResult::ParseErr(err),
        }
    }
