...
const DX_DISABLE_LIBDEFLATE_EXTRACT_POOL_ENV: &str = "BUN_DX_DISABLE_LIBDEFLATE_EXTRACT_POOL";
...
struct LibdeflateDecompressorSlot(Cell<Option<NonNull<libdeflate::Decompressor>>>);

impl Drop for LibdeflateDecompressorSlot {
    fn drop(&mut self) {
        if let Some(ptr) = self.0.take() {
            // SAFETY: the pointer was allocated by libdeflate and is parked in
            // this thread-local slot only when no PooledLibdeflateDecompressor owns it.
            unsafe { libdeflate::Decompressor::destroy(ptr.as_ptr()) };
        }
    }
}

thread_local! {
    static LIBDEFLATE_DECOMPRESSOR: LibdeflateDecompressorSlot =
        const { LibdeflateDecompressorSlot(Cell::new(None)) };
}

struct PooledLibdeflateDecompressor {
    ptr: NonNull<libdeflate::Decompressor>,
    pooled: bool,
}

impl PooledLibdeflateDecompressor {
    fn get() -> Option<Self> {
        libdeflate::load();
        if dx_disable_libdeflate_extract_pool() {
            return NonNull::new(libdeflate::Decompressor::alloc())
                .map(|ptr| Self { ptr, pooled: false });
        }

        LIBDEFLATE_DECOMPRESSOR.with(|slot| {
            let ptr = match slot.0.take() {
                Some(ptr) => ptr,
                None => NonNull::new(libdeflate::Decompressor::alloc())?,
            };
            Some(Self { ptr, pooled: true })
        })
    }

    #[inline]
    fn as_mut(&mut self) -> &mut libdeflate::Decompressor {
        // SAFETY: the guard owns the decompressor until Drop returns it to TLS.
        unsafe { self.ptr.as_mut() }
    }
}

impl Drop for PooledLibdeflateDecompressor {
    fn drop(&mut self) {
        if !self.pooled {
            // SAFETY: this handle was allocated for a one-shot disabled-pool
            // probe and is still uniquely owned by the guard.
            unsafe { libdeflate::Decompressor::destroy(self.ptr.as_ptr()) };
            return;
        }

        LIBDEFLATE_DECOMPRESSOR.with(|slot| {
            if slot.0.get().is_some() {
                // SAFETY: recursion restored a different decompressor, so this
                // guard still uniquely owns its handle and must free it.
                unsafe { libdeflate::Decompressor::destroy(self.ptr.as_ptr()) };
            } else {
                slot.0.set(Some(self.ptr));
            }
        });
    }
}
...
#[inline]
fn dx_disable_libdeflate_extract_pool() -> bool {
    static DISABLE: std::sync::LazyLock<bool> = std::sync::LazyLock::new(|| {
        std::env::var_os(DX_DISABLE_LIBDEFLATE_EXTRACT_POOL_ENV).is_some()
    });
    *DISABLE
}
        ...
        )?;
        {
...

            {
                ...
                if tgz_bytes.len() > 16 {
                    ...
                    let last_4_bytes: u32 = u32::from_le_bytes(
            ...
            {
                'use_libdeflate: {
                    let Some(mut decompressor) = PooledLibdeflateDecompressor::get() else {
                        break 'use_libdeflate;
                    };

                    zlib_pool.list.clear();
                    let result = decompressor.as_mut().decompress_to_vec(
                        tgz_bytes,
                        &mut zlib_pool.list,
