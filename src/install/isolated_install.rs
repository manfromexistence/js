...
use bun_collections::{
    ...
    StringArrayHashMap, smallvec::SmallVec,
};
...
pub(crate) fn install_isolated_packages(
    ...
    let store: Store = 'store: {
        ...
        let mut dep_ids_sort_buf: SmallVec<[DependencyID; 16]> = SmallVec::new();
        ...
        if peer_name_count != 0 {
            ...
            let mut peer_targets: Vec<SmallVec<[PackageID; 4]>> =
                vec![SmallVec::new(); peer_name_count as usize];
        ...
        let mut peer_dep_ids: SmallVec<[DependencyID; 8]> = SmallVec::new();

        let mut visited_parent_node_ids: SmallVec<[store::node::Id; 16]> = SmallVec::new();
            ...
            if entry.dep_id != invalid_dependency_id {
                ...
                'dont_dedupe: {
                    ...
                    if dedupe_entry.found_existing {
                        ...
                        let dedupe_node_index = dedupe_node_id.get() as usize;
                        let dedupe_peers: SmallVec<[store::node::TransitivePeer; 4]> =
                            node_peers[dedupe_node_index].list.iter().copied().collect();
