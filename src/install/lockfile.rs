...
include!(concat!(
    env!("OUT_DIR"),
    "/default_trusted_dependencies_list.rs"
));

/// The default list of trusted dependencies is generated by build.rs as static
/// phf tables so package-name lookups avoid first-use allocation.
pub mod default_trusted_dependencies {
    use super::{
        DEFAULT_TRUSTED_DEPENDENCIES, DEFAULT_TRUSTED_DEPENDENCIES_LIST,
        DEFAULT_TRUSTED_DEPENDENCY_HASHES,
    };
    ...
    pub(crate) fn entries() -> impl Iterator<Item = Entry> {
        DEFAULT_TRUSTED_DEPENDENCIES_LIST.iter().map(|&key| Entry {
            hash: u64::from(
                *DEFAULT_TRUSTED_DEPENDENCIES
                    .get(key)
                    .expect("generated trusted dependency table is missing list entry"),
            ),
            key,
        })
    }
    ...
    pub(crate) fn has_with_hash(hash: u64) -> bool {
        match u32::try_from(hash) {
            Ok(hash) => DEFAULT_TRUSTED_DEPENDENCY_HASHES.contains(&hash),
            Err(_) => false,
        }
    }
    ...
    pub(crate) fn has(name: &[u8]) -> bool {
        DEFAULT_TRUSTED_DEPENDENCIES.contains_key(name)
    }
}
