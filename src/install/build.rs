...
fn main() {
    ...
    let mut out = String::from("pub static DEFAULT_TRUSTED_DEPENDENCIES_LIST: &[&[u8]] = &[\n");
    for name in &names {
    ...
    out.push_str(
        "pub static DEFAULT_TRUSTED_DEPENDENCIES: phf::Map<&'static [u8], u32> = phf::phf_map! {\n",
    );
    for name in &names {
        let hash = Wyhash11::hash(0, name.as_bytes()) as u32;
        writeln!(out, "    b\"{name}\" => {hash}u32,").unwrap();
    }
    out.push_str("};\n\n");

    let mut hashes: Vec<u32> = names
        .iter()
        .map(|name| Wyhash11::hash(0, name.as_bytes()) as u32)
        .collect();
    hashes.sort_unstable();
    hashes.dedup();

    out.push_str("pub static DEFAULT_TRUSTED_DEPENDENCY_HASHES: phf::Set<u32> = phf::phf_set! {\n");
    for hash in hashes {
        writeln!(out, "    {hash}u32,").unwrap();
    }
    out.push_str("};\n");
