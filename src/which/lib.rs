...
use bun_paths::{is_absolute, path_buffer_pool, PathBuffer, WPathBuffer, MAX_PATH_BYTES, SEP};
...
fn search_bin_in_path(
    ...
    if segment.len() + 1 + bin_utf16.len() + tail_units > buf.len()
    ...
    let segment = bun_core::strings::without_trailing_slash(segment);
    let segment_utf16_len = if let Some(len) = copy_ascii_to_utf16(&mut buf[..], segment) {
        len
    } else {
        let segment_utf16 =
            bun_core::strings::convert_utf8_to_utf16_in_buffer(&mut buf[..], segment);
        segment_utf16.len()
    };

    buf[segment_utf16_len] = SEP as u16;

    buf[segment_utf16_len + 1..segment_utf16_len + 1 + bin_utf16.len()].copy_from_slice(bin_utf16);
...
fn encode_bin_utf16<'a>(
    buf: &'a mut WPathBuffer,
    bin: &[u8],
    tail_units: usize,
) -> Option<&'a [u16]> {
    if bin.len() + tail_units <= buf.len() {
        if let Some(bin_len) = copy_ascii_to_utf16(&mut buf[..], bin) {
            return Some(&buf[..bin_len]);
        }
    }

    let bin_len = bun_core::strings::element_length_utf8_into_utf16(bin);
    if bin_len + tail_units > buf.len() {
        return None;
    }

    let bin_utf16 = bun_core::strings::convert_utf8_to_utf16_in_buffer(&mut buf[..], bin);
    let bin_utf16_len = bin_utf16.len();
    Some(&buf[..bin_utf16_len])
}

#[inline]
fn copy_ascii_to_utf16(buf: &mut [u16], input: &[u8]) -> Option<usize> {
...
pub fn which_win<'a>(
    ...
    if is_absolute(bin) {
        ...
        let bin_utf16_len = bin_utf16.len();
        buf[bin_utf16_len] = 0;
        return search_bin(buf, bin_utf16_len, check_windows_extensions).map(|len| &buf[..len]);
    }

    // check if bin is in cwd
    if strings::index_of_char(bin, b'/').is_some() || strings::index_of_char(bin, b'\\').is_some() {
        let mut bin_buf = w_path_buffer_pool::get();
        let bin_utf16 = encode_bin_utf16(
            &mut *bin_buf,
            strings::without_prefix_comptime(bin, b"./"),
            if check_windows_extensions { 5 } else { 1 },
        )?;
        if let Some(bin_path_len) = search_bin_in_path(
            buf,
            &mut *path_buf,
            cwd,
            bin_utf16,
            ...
            let bin_path = &mut buf[..bin_path_len];
    ...
    if dx_disable_which_bin_utf16_reuse() {
        for segment_part in path.split(|b| *b == b';').filter(|s| !s.is_empty()) {
            let mut loop_bin_buf = w_path_buffer_pool::get();
            let loop_bin_utf16 = encode_bin_utf16(
                &mut *loop_bin_buf,
                bin,
                if check_windows_extensions { 5 } else { 1 },
            )?;
...
#[cfg(windows)]
#[inline]
fn dx_disable_which_bin_utf16_reuse() -> bool {
    static DISABLE: std::sync::LazyLock<bool> = std::sync::LazyLock::new(|| {
        std::env::var_os("BUN_DX_DISABLE_WHICH_BIN_UTF16_REUSE").is_some()
    });
    *DISABLE
}
