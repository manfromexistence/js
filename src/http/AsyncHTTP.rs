...
fn build_proxy_authorization(proxy: &URL<'_>) -> Option<Vec<u8>> {
    ...
    let username = match decode_proxy_auth_part(proxy.username) {
        Ok(value) => value,
        Err(err) => {
    ...
    let auth: SmallVec<[u8; 512]> = if !proxy.password.is_empty() {
        let password = match decode_proxy_auth_part(proxy.password) {
            Ok(value) => value,
            Err(err) => {
        ...
        let mut auth = SmallVec::with_capacity(username.len() + 1 + password.len());
        auth.extend_from_slice(&username);
        auth.push(b':');
        auth.extend_from_slice(&password);
        auth
    } else {
        let mut auth = SmallVec::with_capacity(username.len());
        auth.extend_from_slice(&username);
        auth
    };
...
#[inline]
fn decode_proxy_auth_part(input: &[u8]) -> Result<SmallVec<[u8; 256]>, bun_url::DecodeError> {
    let mut output = SmallVec::<[u8; 256]>::new();
    output.resize(input.len(), 0);
    let len = PercentEncoding::decode_into(&mut output, input)?;
