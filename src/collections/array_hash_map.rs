...
impl<V, A: Allocator + HashbrownAllocator + Clone + Default> StringHashMap<V, A> {
    ...
    /// Single-probe hit path with fallible miss construction. This is the safe
    /// equivalent of Zig's `getOrPut` shape for caches that must do expensive
    /// work before inserting: occupied entries return immediately from the raw
    /// probe, while vacant entries call `make_value` and insert only on success.
    #[inline]
    pub fn get_or_try_insert_hashed<E>(
        &mut self,
        hash: u64,
        key: &[u8],
        make_value: impl FnOnce() -> Result<V, E>,
    ) -> Result<&mut V, E> {
            ...
            .from_key_hashed_nocheck(hash, key)
        {
            ...
            RawEntryMut::Vacant(entry) => {
                ...
                let (_, value) = entry.insert_hashed_nocheck(hash, owned_key::<A>(key), value);
                Ok(value)
            }
        }
    }

    /// Mutable [`get_hashed`]. This is for hit-heavy caches that already need
    /// the key hash for their miss path and should not pay a second probe or
    /// allocate an owned key just to learn that the value exists.
    #[inline]
...
mod index_tests {
    ...
    #[test]
    fn string_hash_map_get_mut_hashed_updates_existing_entry() {
        let mut m: StringHashMap<usize> = StringHashMap::default();
        m.put(b"package.json", 1).unwrap();

        let hash = m.hash_key(b"package.json");
        *m.get_mut_hashed(hash, b"package.json").unwrap() = 2;

        assert_eq!(m.get(&b"package.json"[..]), Some(&2));
        assert!(m.get_mut_hashed(hash, b"missing").is_none());
    }
}
