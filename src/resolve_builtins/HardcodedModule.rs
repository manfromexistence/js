...
// Alias source tables. `build.rs` parses these and emits PHF maps.
#[allow(dead_code)]
...
include!(concat!(env!("OUT_DIR"), "/hardcoded_aliases.rs"));

#[inline]
fn lookup(map: &phf::Map<&'static [u8], Alias>, name: &[u8]) -> Option<Alias> {
    map.get(name).copied()
}
...
mod tests {
    ...
    #[test]
    fn generated_alias_maps_cover_node_bun_and_test_aliases() {
        assert_eq!(
            Alias::get(b"fs", Target::Node, Cfg::default())
                .unwrap()
                .path
                .as_bytes(),
            b"node:fs"
        ...
        assert_eq!(
            Alias::get(b"node:fs", Target::Node, Cfg::default())
                .unwrap()
                .path
                .as_bytes(),
            b"node:fs"
        ...
        assert_eq!(
            Alias::get(b"ffi", Target::Bun, Cfg::default())
                .unwrap()
                .path
                .as_bytes(),
            b"bun:ffi"
        );
        assert!(Alias::get(b"vitest", Target::Bun, Cfg::default()).is_none());
        assert_eq!(
            Alias::get(
                b"vitest",
                Target::Bun,
                Cfg {
                    rewrite_jest_for_tests: true,
                },
            )
            .unwrap()
            .path
